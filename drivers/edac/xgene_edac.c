				    ...
				    int cpu_idx)
{
	...
	while (val & (MEMERR_CPU_ICFESR_CERR_MASK |
		...
		/* Make sure all errors are gone */
		val = readl(pg_f + MEMERR_CPU_ICFESR_PAGE_OFFSET);
	}

	val = readl(pg_f + MEMERR_CPU_LSUESR_PAGE_OFFSET);
	while (val & (MEMERR_CPU_LSUESR_CERR_MASK |
	...
	while (val & (MEMERR_CPU_MMUESR_CERR_MASK |
...
static void xgene_edac_pmd_l2_check(struct edac_device_ctl_info *edac_dev)
{
	...
	while (val & (MEMERR_L2C_L2ESR_ERR_MASK |
...
+	              MEMERR_L2C_L2ESR_UCERR_MASK |
	...
	while (val & (MEMERR_L2C_L2RTOSR_ERR_MASK |
		      MEMERR_L2C_L2RTOSR_MULTERR_MASK)) {
...
static bool xgene_edac_support_l2rto(void)
{
	...
	switch ((midr & MIDR_EL1_VARIANT_MASK) >> MIDR_EL1_VARIENT_SHIFT) {
	case 0: /* X-Gene1 family */
		switch (midr & MIDR_EL1_REV_MASK) {
		case 0:
			switch (revidr & REVIDR_EL1_MINOR_REV_MASK) {
			case 1: /* A0/A1 */
			case 2: /* A2/A3 */
				return 0;
			};
...
static bool xgene_is_xgene1(void)
{
	u32 val;
