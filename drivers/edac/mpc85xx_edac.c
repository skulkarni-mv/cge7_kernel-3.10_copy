...
#if defined(CONFIG_PCI) && (defined(CONFIG_FSL_SOC_BOOKE) || defined(CONFIG_PPC_86xx))
extern struct of_device_id fsl_pci_ids[];
#endif

static void devm_irq_dispose_mapping(void *p)
{
	unsigned int irq = (unsigned int)((unsigned long)p);

	if (irq)
		irq_dispose_mapping(irq);
}

static int devm_irq_of_parse_and_map(struct device *dev,
			struct device_node *dn, int index)
{
	unsigned int irq;
	int ret;

	irq = irq_of_parse_and_map(dn, index);
	if (!irq)
		return -EINVAL;

	ret = devm_add_action(dev, devm_irq_dispose_mapping,
...
static int mpc85xx_pcie_find_capability(struct device_node *np)
{
	...
	return pci_bus_find_capability(hose->bus, 0, PCI_CAP_ID_EXP);
}
...
static int mpc85xx_pci_err_probe(struct platform_device *op)
{
	...
	if (edac_op_state == EDAC_OPSTATE_INT) {
		...
		res = devm_irq_of_parse_and_map(&op->dev, op->dev.of_node, 0);
		if (!res) {
			printk(KERN_ERR
			       "%s: Unable to locale irq for "
			       "MPC85xx PCI err\n", __func__);
			res = -EINVAL;
			goto err2;
		}
	...
	devres_close_group(&op->dev, mpc85xx_pci_err_probe);
	edac_dbg(3, "success\n");
...
static int __init mpc85xx_mc_init(void)
{
	...
	for_each_matching_node(np, fsl_pci_ids) {
		...
		if (!pdev)
			continue;
		res = mpc85xx_pci_err_probe(pdev);
		if (res)
			dev_warn(&pdev->dev, EDAC_MOD_STR "PCI fails to register\n");
	}
#endif
	...
	if ((PVR_VER(pvr) == PVR_VER_E500V1) ||
		 ...
		 */
		if (edac_op_state == EDAC_OPSTATE_INT)
			on_each_cpu(mpc85xx_mc_clear_rfxe, NULL, 1);
	}
#endif
...
static void __exit mpc85xx_mc_exit(void)
{
	...
	if ((PVR_VER(pvr) == PVR_VER_E500V1) ||
	    (PVR_VER(pvr) == PVR_VER_E500V2)) {
		on_each_cpu(mpc85xx_mc_restore_hid1, NULL, 1);
	}
#endif
