...
#ifdef CONFIG_ARM64
static irqreturn_t ahci_interrupt_with_errata(int irq, void *dev_instance)
{
	struct ata_host *host = dev_instance;
	struct ahci_host_priv *hpriv;
	unsigned int i, handled = IRQ_NONE;
...
#ifdef CONFIG_ARM64
	else if (cpus_have_cap(ARM64_WORKAROUND_CAVIUM_22536))
		rc = ata_host_activate(host, irq, ahci_interrupt_with_errata,
				       IRQF_SHARED, sht);
#endif
	else
