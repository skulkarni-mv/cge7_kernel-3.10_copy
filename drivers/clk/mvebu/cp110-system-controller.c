...
static int cp110_gate_enable(struct clk_hw *hw)
{
	...
	if (gate->lock)
        ...
        reg = readl_relaxed(base + CP110_PM_CLOCK_GATING_REG);

        reg |= BIT(gate->bit_idx);

        writel_relaxed(reg, base + CP110_PM_CLOCK_GATING_REG);
...
static void cp110_gate_disable(struct clk_hw *hw)
{
	...
	reg_disable = readl_relaxed(base + CP110_PM_CLOCK_GATING_REG);

	reg_disable &= ~BIT(gate->bit_idx);

	writel_relaxed(reg_disable, base + CP110_PM_CLOCK_GATING_REG);
...
static int cp110_syscon_clk_probe(struct platform_device *pdev)
{
	...
	nand_clk_ctrl = readl_relaxed(base);
	...
	for (i = 0; i < CP110_MAX_GATABLE_CLOCKS; i++) {
		...
		clk = cp110_register_gate(name, parent, base, i);
		if (IS_ERR(clk)) {
			ret = PTR_ERR(clk);
			goto fail_gate;
		}
...
MODULE_DEVICE_TABLE(of, cp110_syscon_of_match);
...
static int __init cp110_clk_init(void)
{
        return platform_driver_register(&cp110_syscon_driver);
}

arch_initcall(cp110_clk_init);
