...
static bool xgene_clk_is_storm(void)
{
...
static int xgene_clk_enable(struct clk_hw *hw)
{
	...
	if (pclk->param.csr_reg != NULL) {
		...
		xgene_clk_write(data, pclk->param.csr_reg +
					pclk->param.reg_clk_offset);
		pr_debug("%s clock PADDR base 0x%016llx clk offset 0x%08X mask 0x%08X value 0x%08X\n",
		...
		xgene_clk_write(data, pclk->param.csr_reg +
					pclk->param.reg_csr_offset);
		pr_debug("%s CSR RESET PADDR base 0x%016llx csr offset 0x%08X mask 0x%08X value 0x%08X\n",
			pclk->name, __pa(pclk->param.csr_reg),
