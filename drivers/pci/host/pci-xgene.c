...
#define OMR1MSKL			0x108
#define OMR2MSKL			0x120
#define OMR3MSKL			0x138
...
#define XGENE_PCIE_DEV_CTRL		0x2f0A
...
#define XGENE_PCIE_EVENT_INTR_STS	0x58
#define XGENE_PCIE_EVENT_INTR_MSK	0x5C
#define PTYPE_ENDPOINT			0
#define PTYPE_ROOT_PORT			1
#define XGENE_PCIE_EVENT_IRQ_OFFSET     4
#define XGENE_AMA_MODE_BASE 		0x1F2A0000
#define XGENE_AMA_MODE_OFFSET 		0xE014

#define GLBL_TRANS_ERR			0xD860
#define INT_SLV_TMO			0xE00C
#define SWINT				0x70
#define ROOT_PORT_CAP_AER		0x100
#define ROOT_PORT_AER_ERR_CMD_REG	0x2c
#define ROOT_PORT_AER_COR_EN_MASK	0x1
#define MAX_PORTS 5
...
static struct xgene_pcie_port_info {
	void __iomem *csr_base;
	void __iomem *cfg_base;
} pcie_ports[MAX_PORTS];
...
int xgene_pcie_get_port_index(struct pci_bus *bus)
{
	struct xgene_pcie_port *port = bus->sysdata;

	return port->index;
}
EXPORT_SYMBOL(xgene_pcie_get_port_index);
...
u32 xgene_pcie_dma_read_int_msk(int port)
{
	return readl(pcie_ports[port].csr_base + XGENE_PCIE_EVENT_INTR_MSK);
}
EXPORT_SYMBOL(xgene_pcie_dma_read_int_msk);
void xgene_pcie_dma_write_int_msk(int port, u32 val)
{
	writel(val, pcie_ports[port].csr_base + XGENE_PCIE_EVENT_INTR_MSK);
}
EXPORT_SYMBOL(xgene_pcie_dma_write_int_msk);
u32 xgene_pcie_dma_read_int_sts(int port)
{
	return readl(pcie_ports[port].csr_base + XGENE_PCIE_EVENT_INTR_STS);
}
EXPORT_SYMBOL(xgene_pcie_dma_read_int_sts);
void xgene_pcie_dma_write_int_sts(int port, u32 val)
{
	writel(val, pcie_ports[port].csr_base + XGENE_PCIE_EVENT_INTR_STS);
}
EXPORT_SYMBOL(xgene_pcie_dma_write_int_sts);
void __iomem *xgene_pcie_dma_cfg_base(int port)
{
	return ((pcie_ports[port].cfg_base == NULL) ? NULL :
			pcie_ports[port].cfg_base + 0x20000);
}
EXPORT_SYMBOL(xgene_pcie_dma_cfg_base);
...
static void xgene_pcie_program_core(void __iomem *csr_base)
{
	...
	val = readl(csr_base + 0x18);
	val |= 0x60000;
	writel(val, csr_base + 0x18);
}
			      ...
			      struct platform_device *pdev, u64 *cfg_addr)
{
	...
	*cfg_addr = res->start;
	...
	if (!strcmp("1f2b0000.pcie", pdev->name))
		index = 0;
	if (!strcmp("1f2c0000.pcie", pdev->name))
		index = 1;
	if (!strcmp("1f2d0000.pcie", pdev->name))
		index = 2;
	if (!strcmp("1f500000.pcie", pdev->name))
		index = 3;
	if (!strcmp("1f510000.pcie", pdev->name))
		index = 4;

	if (index != -1) {
		pcie_ports[index].cfg_base = port->cfg_base;
		pcie_ports[index].csr_base = port->csr_base;
		port->index = index;
	} else
		dev_info(port->dev, "Device name %s does not match\n",
				pdev->name);
				 ...
				 resource_size_t io_base)
{
	...
	/*
	 * Disable all OMRs and enable only those that are required.
	 */
	writel(0x0, port->csr_base + OMR1MSKL);
	writel(0x0, port->csr_base + OMR2MSKL);
	writel(0x0, port->csr_base + OMR3MSKL);
	...
	list_for_each_entry(window, &bridge->windows, list) {
		...
		case IORESOURCE_IO:
			xgene_pcie_setup_ob_reg(port, res, OMR3BARL,
						io_base);
			ret = pci_ioremap_io(res, io_base);
			if (ret < 0)
				return ret;
			break;
		case IORESOURCE_MEM:
...
int xgene_pcie_map_irq(const struct pci_dev *pci_dev, u8 slot, u8 pin)
{
        ...
        u32 virq;
        int ret;
        ret = of_irq_parse_pci((struct pci_dev *) pci_dev, &oirq);
        if (ret)
                return ret;
...
static void xgene_pcie_disable_corrected_error_aer(struct xgene_pcie_port *port)
{
	/* AER: Disable corrected error reporting */
	u32 val = readl(port->cfg_base +
			  ROOT_PORT_CAP_AER + ROOT_PORT_AER_ERR_CMD_REG);
	val &= ~ROOT_PORT_AER_COR_EN_MASK;
	writel(val, port->cfg_base +
			  ROOT_PORT_CAP_AER + ROOT_PORT_AER_ERR_CMD_REG);
}

int xgene_pcie_validate_bars(struct xgene_pcie_port *port)
{
	void *csr_base = port->csr_base;
	u64 omr1msk = 0, omr2msk = 0, omr3msk = 0;
	u64 omr1bar = 0, omr2bar = 0, omr3bar = 0;
	u64 pom1 = 0, pom2 = 0, pom3 = 0;
	u64 cfgbar = 0;

	omr1msk = ((u64)readl(csr_base + OMR1MSKL + 4) << 32) | readl(csr_base + OMR1MSKL);
	omr2msk = ((u64)readl(csr_base + OMR2MSKL + 4) << 32) | readl(csr_base + OMR2MSKL);
	omr3msk = ((u64)readl(csr_base + OMR3MSKL + 4) << 32) | readl(csr_base + OMR3MSKL);
	cfgbar = ((u64)readl(csr_base + CFGBARH) << 32) | readl(csr_base + CFGBARL);

	if (omr1msk & EN_REG) {
		omr1bar = ((u64)readl(csr_base + OMR1BARL + 4) << 32) | 
			readl(csr_base + OMR1BARL);
		pom1 = ((u64)readl(csr_base + OMR1MSKL + 0x14) << 32) | 
			readl(csr_base + OMR1MSKL + 0x10);
		dev_dbg(port->dev, "%llx %llx %llx\n", omr1msk, omr1bar, pom1);
		if (!omr1bar || ((omr1bar & omr1msk) != omr1bar) || 
				((pom1 & omr1msk) != pom1)) {
			dev_err (port->dev, "Invalid OMR1 settings\n");
			goto out_err;
		}
		omr1bar = omr1bar & omr1msk;
	}
	if (omr2msk & EN_REG) {
		omr2bar = ((u64)readl(csr_base + OMR2BARL + 4) << 32) | 
			readl(csr_base + OMR2BARL);
		pom2 = ((u64)readl(csr_base + OMR2MSKL + 0x14) << 32) | 
			readl(csr_base + OMR2MSKL + 0x10);
		dev_dbg(port->dev, "%llx %llx %llx\n", omr2msk, omr2bar, pom2);
		if (!omr2bar || ((omr2bar & omr2msk) != omr2bar) ||
				((pom2 & omr2msk) != pom2)) {
			dev_err (port->dev, "Invalid OMR2 settings %d %d %d\n",
					!omr2bar ,
					(omr2bar & omr2msk) != omr2bar,
					(pom2 & omr2msk) != pom2);
			goto out_err;
		}
		omr2bar = omr2bar & omr2msk;
	}
	if (omr3msk & EN_REG) {
		omr3bar = ((u64)readl(csr_base + OMR3BARL + 4) << 32) | 
			readl(csr_base + OMR3BARL);
		pom3 = ((u64)readl(csr_base + OMR3MSKL + 0x14) << 32) | 
			readl(csr_base + OMR3MSKL + 0x10);
		dev_dbg(port->dev, "%llx %llx %llx\n", omr3msk, omr3bar, pom3);
		if (!omr3bar || ((omr3bar & omr3msk) != omr3bar) ||
				((pom3 & omr3msk) != pom3)) {
			dev_err (port->dev, "Invalid OMR3 settings\n");
			goto out_err;
		}
		omr3bar = omr3bar & omr3msk;
	}
	if (omr1bar) {
		if ((omr2bar && (omr1bar == omr2bar)) || 
				(omr3bar && (omr1bar == omr3bar))) {
			dev_err(port->dev, "OMR1 overlapping\n");
			goto out_err;
		}
		if (omr1bar == cfgbar) {
			dev_err (port->dev, "OMR1 and CFG overlap\n");
			goto out_err;
		}
	}
	if (omr2bar) {
		if (omr3bar && (omr2bar == omr3bar)) {
			dev_err(port->dev, "OMR2 and OMR3 overlap\n");
			goto out_err;
		}
		if (omr2bar == cfgbar) {
			dev_err(port->dev, "OMR2 and CFG overlap\n");
			goto out_err;
		}
	}
	if (omr3bar && (omr3bar == cfgbar)) {
		dev_err(port->dev, "OMR3 and CFG overlap\n");
		goto out_err;
	}
	/* TBD Message Bar */
	return 0;
...
static int xgene_pcie_probe_bridge(struct platform_device *pdev)
{
	...
	if (ret)
        ...
        bus = pci_create_root_bus(&pdev->dev, primary_bus, &xgene_pcie_ops, port, &res);
        if (!bus) {
                ret = -ENODEV;
                goto err_root_bus;
        }

	ret = xgene_pcie_map_ranges(port, bus, cfg_addr, io_base);
	if (ret)
                goto err_root_bus;
	...
	ret = xgene_pcie_validate_bars(port);
	if (ret)
                goto err_root_bus;
