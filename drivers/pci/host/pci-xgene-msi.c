...
static u32 xgene_msi_pci_requester_id(struct pci_dev *dev, u16 alias)
{
...
int arch_setup_msi_irqs(struct pci_dev *pdev, int nvec, int type)
{
...
#endif
	{
		...
		if (np) {
			...
			dev_dbg(&pdev->dev, "Found no xgene,msi phandle\n");
	}
...
/* Return 1 for Storm */
static inline int xgene_is_storm(void)
{
	u32 val;

	asm volatile("mrs %0, midr_el1" : "=r" (val));
	return (!(val & 0x00f00000));
}
...
static int xgene_msi_probe(struct platform_device *pdev)
{
	...
	set_its_pci_requester_id(xgene_msi_pci_requester_id);
	...
	if (!xgene_is_storm()) {
		/* specifically disable v2m msi gic */
		data = readl(msi->msi_regs + MSI_GICV2_CFG);
		writel(data | 0x1, msi->msi_regs + MSI_GICV2_CFG);
	}
