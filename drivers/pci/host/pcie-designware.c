...
#if defined(CONFIG_LS1_MSI) || defined(CONFIG_ARCH_MVEBU)
static int dw_pcie_msi_ctrl_init(struct pcie_port *pp)
{
	...
	if (msi_node) {
		...
		dev_err(pp->dev, "Cannot find msi node\n");
		return -ENODEV;
	}
...
int __init dw_pcie_host_init(struct pcie_port *pp)
{
	...
	for_each_of_pci_range(&parser, &range) {
		...
		if (restype == IORESOURCE_IO) {
			...
			pp->io.end = min_t(resource_size_t, IO_SPACE_LIMIT, pp->io.start + SZ_64K - 1);
#else
		...
		if (restype == IORESOURCE_MEM) {
			int idx = pp->mem_count;
			if (idx >= MAX_PCI_MEM_COUNT)
				continue;

			of_pci_range_to_resource(&range, np,
						 &pp->mem[idx].mem);
			pp->mem[idx].mem.name = "MEM";
			pp->mem[idx].mem_size =
					resource_size(&pp->mem[idx].mem);
			pp->mem[idx].mem_bus_addr = range.pci_addr;
			pp->mem[idx].mem_base = pp->mem[idx].mem.start;

			/* Find the untranslated MEM space address */
			pp->mem[idx].mem_mod_base =
	...
	if (IS_ENABLED(CONFIG_PCI_MSI) && !IS_ENABLED(CONFIG_LS1_MSI)) {
	...
	if (of_property_read_u32(np, "num-atus", &pp->atu_num))
		pp->atu_num = 2;

	if (pp->atu_num >= 4) {
		pp->atu_idx[ATU_TYPE_CFG0] = PCIE_ATU_REGION_INDEX0;
		pp->atu_idx[ATU_TYPE_CFG1] = PCIE_ATU_REGION_INDEX1;
		pp->atu_idx[ATU_TYPE_MEM] = PCIE_ATU_REGION_INDEX3;
		pp->atu_idx[ATU_TYPE_IO] = PCIE_ATU_REGION_INDEX2;
	} else {
		pp->atu_idx[ATU_TYPE_CFG0] = PCIE_ATU_REGION_INDEX0;
...
static void dw_pcie_prog_viewport_mem_outbound(struct pcie_port *pp)
{
	...
	for (i = 0; i < pp->mem_count; i++) {
		atu_idx = pp->atu_idx[ATU_TYPE_MEM] + i;
		if (atu_idx >= pp->atu_num)
			break;

		/* Program viewport : OUTBOUND : MEM */
		dw_pcie_writel_rc(pp,
				  PCIE_ATU_REGION_OUTBOUND | atu_idx,
				  PCIE_ATU_VIEWPORT);
		dw_pcie_writel_rc(pp, PCIE_ATU_TYPE_MEM, PCIE_ATU_CR1);
		dw_pcie_writel_rc(pp, pp->mem[i].mem_mod_base,
				  PCIE_ATU_LOWER_BASE);
		dw_pcie_writel_rc(pp, pp->mem[i].mem_mod_base >> 32,
				  PCIE_ATU_UPPER_BASE);
		dw_pcie_writel_rc(pp, pp->mem[i].mem_mod_base +
				  pp->mem[i].mem_size - 1,
				  PCIE_ATU_LIMIT);
		dw_pcie_writel_rc(pp, pp->mem[i].mem_bus_addr,
				  PCIE_ATU_LOWER_TARGET);
		dw_pcie_writel_rc(pp, upper_32_bits(pp->mem[i].mem_bus_addr),
...
static int dw_pcie_setup(int nr, struct pci_sys_data *sys)
{
...
#ifdef CONFIG_ARM64
#ifndef CONFIG_ARCH_MVEBU /* PCI IO is broken on 7040 rz */
		pci_ioremap_io(&pp->io, pp->io_base);
#endif
#else
	...
	sys->mem_offset = pp->mem[0].mem.start - pp->mem[0].mem_bus_addr;
	for (i = 0; i < pp->mem_count; i++)
		pci_add_resource_offset(&sys->resources, &pp->mem[i].mem,
					sys->mem_offset);
