...
{
	int msi;
	struct iproc_pcie *pcie = chip->pcie;

	msi = find_first_zero_bit(&chip->irq_in_use, pcie->max_bus_irqs);
	if (msi < chip->pcie->max_bus_irqs)
		...
		struct pci_dev *pdev, struct msi_desc *desc)
{
	...
	/* Spread the interrupts over the EQs */
	addr = virt_to_phys((void *)msi->msi_page[0]) |
			    ((hwirq % pcie->max_irqs) * 4);
	msg.address_lo = addr & 0xffffffff;
	msg.address_hi = (u64) addr >> 32;
	msg.data = irq;
...
static void iproc_msi_teardown_irq(struct msi_controller *chip, unsigned int irq)
{
	struct iproc_msi *msi = to_iproc_msi(chip);
...
static irqreturn_t iproc_msi_irq(int irq, void *data)
{
	...
	do {
		u32 vec = *(u32 *)(msi->eq_page[0] + (eq * 0x100) + (head * 4));
...
static int iproc_msix_setup_irq(struct msi_controller *chip,
		struct pci_dev *pdev, struct msi_desc *desc)
{
	struct iproc_msi *msi = to_iproc_msi(chip);
	struct iproc_pcie *pcie = msi->pcie;
...
static void iproc_msix_teardown_irq(struct msi_controller *chip, unsigned int irq)
{
	struct iproc_msi *msi = to_iproc_msi(chip);
	struct irq_data *data = irq_get_irq_data(irq);
...
static int iproc_pcie_msix_init(struct iproc_pcie *pcie)
{
	...
	for (i = 0; i < pcie->soc_data->num_pf; i++) {
		...
		writel((u64) virt_to_phys((void *)msi->eq_page[i]) >> 32,
		...
		writel((u64) virt_to_phys((void *)msi->msi_page[i]) >> 32,
...
#ifdef CONFIG_PCIE_IPROC_MSIX
	pcie->max_bus_irqs = MAX_MSI_IRQS;
#else
	pcie->max_bus_irqs = pcie->max_irqs;
#endif
...
#include <linux/phy/cmic_mdio.h>
		...
		cmic_mdio_write(INTERNAL, CLAUS22, 0, 0, 0x03, 0x2b18);
		cmic_mdio_write(INTERNAL, CLAUS22, 7, 0, 0x1F, 0x2100);
		cmic_mdio_write(INTERNAL, CLAUS22, 7, 0, 0x03, 0x2b18);
#endif
	}
...
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_BROADCOM, 0xe711, northstar_ns2_omap_fix);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_BROADCOM, 0xd712, northstar_ns2_omap_fix);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_BROADCOM, 0xd713, northstar_ns2_omap_fix);
DECLARE_PCI_FIXUP_FINAL(PCI_VENDOR_ID_BROADCOM, 0xd715, northstar_ns2_omap_fix);
