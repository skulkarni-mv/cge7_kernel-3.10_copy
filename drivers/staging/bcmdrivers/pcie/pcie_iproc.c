...
#define MAX_PCIE_IRQ 6
static unsigned int pcie_dev_cnt=0;
...
static int __maybe_unused soc_pcie_serdes_reg_read(int phyaddr, int reg, u16 *val)
{
	...
	iproc_mii_read(MII_DEV_LOCAL, phyaddr, reg&0xf, val);
...
static void __maybe_unused __init noinline soc_pcie_bridge_init(struct soc_pcie_port * port)
{
	...
	*/
    ...
    pci_bus_write_config_word(&bus, devfn, PCI_MEMORY_LIMIT, (port->owin_res->end) >> 16 );
...
void WrongPCIGen2TemplateWAR(int port, u32 reg, u16 val)
{
	/* port = phy addr */
...
void pcie_msi_init(struct soc_pcie_port *pcie_port)
{
	unsigned j, k=SOC_PCIE_MSI_EQS_ENABLE;
	unsigned int eqRegVal=0, intVal, *lptr;
	int ret = 0;
	struct soc_pcie_port *port = pcie_port;
...
static int iproc_pcie_probe(struct platform_device *pdev)
{
	...
	struct soc_pcie_port *port = &soc_pcie_ports[pcie_dev_cnt];
	int i, rlen;
	...
	if (!base_addr) {
		dev_err(&pdev->dev, "can't iomap pcie base address\n");
		return -ENOMEM;
	}
	port->reg_base = base_addr;
	...
	/*Currently only get 32 bit addr and size.*/
	soc_pcie_owin[pcie_dev_cnt-1].start = of_read_number(ranges + 2, 1);
	soc_pcie_owin[pcie_dev_cnt-1].end = soc_pcie_owin[pcie_dev_cnt-1].start + of_read_number(ranges + 5, 1) - 1;
	soc_pcie_owin[pcie_dev_cnt-1].flags = IORESOURCE_MEM;
	port->owin_res = &soc_pcie_owin[pcie_dev_cnt-1];
	
	/*Parse IRQ*/ 	
  	irqs_total = of_irq_count(np);	
  	if ( !irqs_total || (irqs_total > MAX_PCIE_IRQ) )
  		return -EINVAL;
 		
	for (i = 0; i < irqs_total; i++) {
		port->irqs[i] = irq_of_parse_and_map(np, i);
		if (!port->irqs[i]) {
			dev_err(&pdev->dev, "unable to parse irq index:%d\n", i);
			return -EINVAL;
		}
	}

  	port->hw_pci.domain = pcie_dev_cnt - 1;

#if (defined(CONFIG_MACH_HX4) || defined(CONFIG_MACH_KT2) || defined(CONFIG_MACH_HR2))  \
	
//  	if (of_device_is_compatible(np, "iproc-p2") || of_device_is_compatible(np, "iproc-p6")) {
  		WrongPCIGen2TemplateWAR(pcie_dev_cnt-1, 0x13, 0x190);
		WrongPCIGen2TemplateWAR(pcie_dev_cnt-1, 0x19, 0x191);
//  	}
#endif
