...
#if defined(CONFIG_MACH_SB2)
	cmdcfg |= (CC_OT | CC_OR);
	/* Enable ENA_EXT_CONFIG only for MAC0, and not for MAC1 (MII) */
	if(ch->etc->unit == 0)
		cmdcfg |= CC_AE;
...
chipphyrd(ch_t *ch, uint phyaddr, uint reg)
{
    ...
    if (int_bus) {
        ...
        /* external phy */
#if (!defined(CONFIG_MACH_SB2) && !defined(CONFIG_MACH_KT2))
...
#elif (defined(CONFIG_MACH_SB2) || defined(CONFIG_MACH_KT2))
  serdes_init(ch->etc->unit, phyaddr);
#elif defined(CONFIG_MACH_HR2)
