...
serdes_init(uint eth_num, uint phyaddr)
{
    ...
    /* Auto Negotiation 10M/100M/1G ¡V SGMII master (for T1014) */
    ...
    iproc_mii_write(MII_DEV_LOCAL, phyaddr, 0x0010, 0x0120);
    iproc_mii_write(MII_DEV_LOCAL, phyaddr, 0x001f, 0x8000);
  ...
  //quick n' dirty way of setting up iproc - t1014 link                                 
  if (1 == eth_num) { 
