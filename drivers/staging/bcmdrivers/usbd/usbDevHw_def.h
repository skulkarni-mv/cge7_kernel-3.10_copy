...
static volatile usbDevHw_REG_t *usbDevHw_REG_P;
...
usbd_reg32_read(volatile uint32_t *reg)
{
    return (le32_to_cpu(*reg));
}

static inline void
usbd_reg32_write(volatile uint32_t *reg, uint32_t value)
{
    *reg = cpu_to_le32(value);
}

static inline void
usbd_reg32_modify_or(volatile uint32_t *reg, uint32_t value)
{
    uint32_t tmp;
    tmp = le32_to_cpu(*reg);
    tmp |= value;
...
static inline void
usbd_reg32_modify_and(volatile uint32_t *reg, uint32_t value)
{
    uint32_t tmp;
    tmp = le32_to_cpu(*reg);
    tmp &= value;
...
static inline void
usbd_reg32_modify_mask(volatile uint32_t *reg, uint32_t mask, uint32_t value)
{
    uint32_t tmp;
    tmp = le32_to_cpu(*reg);
    tmp &= mask;
...
#define REG_RD(reg)                   usbd_reg32_read(&reg)
#define REG_WR(reg, val)              usbd_reg32_write(&reg, val)
#define REG_MOD_OR(reg, val)          usbd_reg32_modify_or(&reg, val)
#define REG_MOD_AND(reg, val)         usbd_reg32_modify_and(&reg, val)
#define REG_MOD_MASK(reg, mask, val)  usbd_reg32_modify_mask(&reg, mask, val)
...
{
	return(REG_RD(usbDevHw_REG_P->devStatus) & usbDevHw_REG_DEV_STATUS_BUS_SUSPENDED ? true : false);
}
...
{
	return((REG_RD(usbDevHw_REG_P->devStatus) & usbDevHw_REG_DEV_STATUS_ALT_NUM_MASK) >>  usbDevHw_REG_DEV_STATUS_ALT_NUM_SHIFT);
}

static inline unsigned usbDevHw_DeviceCfgNum(void)
{
	return((REG_RD(usbDevHw_REG_P->devStatus) & usbDevHw_REG_DEV_STATUS_CFG_NUM_MASK) >>  usbDevHw_REG_DEV_STATUS_CFG_NUM_SHIFT);
}

static inline unsigned usbDevHw_DeviceIntfNum(void)
{
	return((REG_RD(usbDevHw_REG_P->devStatus) & usbDevHw_REG_DEV_STATUS_INTF_NUM_MASK) >>  usbDevHw_REG_DEV_STATUS_INTF_NUM_SHIFT);
}
...
{
	return (REG_RD(usbDevHw_REG_P->devCtrl) & usbDevHw_REG_DEV_CTRL_DMA_OUT_ENABLE ? true : false);
}
...
{
	return((REG_RD(usbDevHw_REG_P->devStatus) & usbDevHw_REG_DEV_STATUS_SOF_FRAME_NUM_MASK) >> usbDevHw_REG_DEV_STATUS_SOF_FRAME_NUM_SHIFT);
}
...
{
	return(REG_RD(usbDevHw_REG_P->devIntrStatus));
}
...
{
	return((~REG_RD(usbDevHw_REG_P->devIntrMask)) & usbDevHw_DEVICE_IRQ_ALL);
}
...
{
	return(REG_RD(usbDevHw_REG_P->devStatus) & usbDevHw_REG_DEV_STATUS_PHY_ERROR ? true : false);
}
...
{
	return(REG_RD(usbDevHw_REG_P->devStatus) & usbDevHw_REG_DEV_STATUS_REMOTE_WAKEUP_ALLOWED ? true : false);
}
...
{
	switch(REG_RD(usbDevHw_REG_P->devStatus) & usbDevHw_REG_DEV_STATUS_SPD_MASK)
	{
...
static inline void usbDevHw_EndptOpsInit(unsigned num, unsigned type, unsigned dirn, unsigned maxPktSize)
{
	...
	if ((type == usbDevHw_ENDPT_TYPE_CTRL) || (dirn == usbDevHw_ENDPT_DIRN_OUT)) {
		REG_WR(usbDevHw_REG_P->eptFifoOut[num].ctrl,   (type << usbDevHw_REG_ENDPT_FIFO_CTRL_TYPE_SHIFT)); /*| */
								   ...
								   */
		REG_WR(usbDevHw_REG_P->eptFifoOut[num].status, REG_RD(usbDevHw_REG_P->eptFifoOut[num].status));
...
{
	if (dirn == usbDevHw_ENDPT_DIRN_OUT) {
		return((REG_RD(usbDevHw_REG_P->eptFifoOut[num].size1) & usbDevHw_REG_ENDPT_FIFO_SIZE1_OUT_FRAME_NUM_MASK) >> usbDevHw_REG_ENDPT_FIFO_SIZE1_OUT_FRAME_NUM_SHIFT);
	}
	return(0);
}
...
{
	if (dirn == usbDevHw_ENDPT_DIRN_OUT) {
		return((REG_RD(usbDevHw_REG_P->eptIntrStatus) & usbDevHw_REG_ENDPT_INTR_OUT_MASK) >> usbDevHw_REG_ENDPT_INTR_OUT_SHIFT);
	}
	return((REG_RD(usbDevHw_REG_P->eptIntrStatus) & usbDevHw_REG_ENDPT_INTR_IN_MASK) >> usbDevHw_REG_ENDPT_INTR_IN_SHIFT);
}
...
static inline uint32_t usbDevHw_EndptStatusActive(unsigned num, unsigned dirn)
{
	if (dirn == usbDevHw_ENDPT_DIRN_OUT) {
		...
		return(REG_RD(usbDevHw_REG_P->eptFifoOut[num].status));  /* End Point Status register */
	}
	return(REG_RD(usbDevHw_REG_P->eptFifoIn[num].status));
}
...
{
	if (dirn == usbDevHw_ENDPT_DIRN_OUT) {
		return (REG_RD(usbDevHw_REG_P->eptFifoOut[num].ctrl) & usbDevHw_REG_ENDPT_FIFO_CTRL_NAK_IN_PROGRESS ? true : false);
	}
	return (REG_RD(usbDevHw_REG_P->eptFifoIn[num].ctrl) & usbDevHw_REG_ENDPT_FIFO_CTRL_NAK_IN_PROGRESS ? true : false);
}
...
{
#if usbDevHw_REG_MULTI_RX_FIFO
	if (!(REG_RD(usbDevHw_REG_P->eptFifoOut[num].status) & usbDevHw_REG_ENDPT_FIFO_STATUS_OUT_FIFO_EMPTY))
#else
	if (!(REG_RD(usbDevHw_REG_P->devStatus) & usbDevHw_REG_DEV_STATUS_OUT_FIFO_EMPTY))
#endif
		return;
...
static inline void usbDevHw_PrintInfo(int (*printFP) (const char *, ...))
{
	...
	printFP("Intrpt devStat=0x%08x devMask=0x%08x epStat=0x%08x epMask=0x%08x\n",
			  REG_RD(usbDevHw_REG_P->devIntrStatus), REG_RD(usbDevHw_REG_P->devIntrMask), REG_RD(usbDevHw_REG_P->eptIntrStatus), REG_RD(usbDevHw_REG_P->eptIntrMask));

	for (num = 0; num < usbDevHw_ENDPT_CFG_CNT; num++) {
		printFP("\nep%d:  cfg=0x%08x\n", num, REG_RD(usbDevHw_REG_P->eptCfg[num]));
		printFP("  i: ctrl=0x%08x stat=0x%08x size1=0x%08x size2=0x%08x setup=0x%08x data=0x%08x\n",
					   REG_RD(usbDevHw_REG_P->eptFifoIn[num].ctrl), REG_RD(usbDevHw_REG_P->eptFifoIn[num].status), REG_RD(usbDevHw_REG_P->eptFifoIn[num].size1),
					   REG_RD(usbDevHw_REG_P->eptFifoIn[num].size2), REG_RD(usbDevHw_REG_P->eptFifoIn[num].setupBufAddr), REG_RD(usbDevHw_REG_P->eptFifoIn[num].dataDescAddr));
		printFP("  o: ctrl=0x%08x stat=0x%08x size1=0x%08x size2=0x%08x setup=0x%08x data=0x%08x\n",
					   REG_RD(usbDevHw_REG_P->eptFifoOut[num].ctrl), REG_RD(usbDevHw_REG_P->eptFifoOut[num].status), REG_RD(usbDevHw_REG_P->eptFifoOut[num].size1),
					   REG_RD(usbDevHw_REG_P->eptFifoOut[num].size2), REG_RD(usbDevHw_REG_P->eptFifoOut[num].setupBufAddr), REG_RD(usbDevHw_REG_P->eptFifoOut[num].dataDescAddr));
	}
}
