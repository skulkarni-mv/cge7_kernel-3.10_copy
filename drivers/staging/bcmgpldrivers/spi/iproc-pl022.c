				    ...
				    ssp_clock_params * clk_freq)
{
	...
	rate =  APH_CLK; //clk_get_rate(pl022->clk);
	/* cpsdvscr = 2 & scr 0 */
...
pl022_probe(struct platform_device *adev)
{
	 ...
	 */
	master->mode_bits = SPI_MODE_3| SPI_NO_CS | SPI_LOOP;
	if (pl022->vendor->extended_cr)
		master->mode_bits |= SPI_LSB_FIRST;

	dev_dbg(&adev->dev, "BUSNO: %d\n", master->bus_num);

	status = request_mem_region(adev->resource->start, resource_size(adev->resource), adev->name);
	
	if (!status)
		goto err_no_ioregion;

	pl022->phybase = adev->resource->start;
	pl022->virtbase = ioremap(adev->resource->start, resource_size(adev->resource));
	if (pl022->virtbase == NULL) {
		status = -ENOMEM;
		goto err_no_ioremap;
	}
	printk(KERN_INFO "pl022: mapped registers from 0x%08x to %p\n",
	       adev->resource->start, pl022->virtbase);
	...
	dev_set_drvdata(&adev->dev, pl022);
...
//	clk_put(pl022->clk);
 ...
 err_no_ioremap:
	release_mem_region(adev->resource->start,resource_size(adev->resource));
 err_no_ioregion:
...
pl022_remove(struct platform_device *adev)
{
    int irq;
