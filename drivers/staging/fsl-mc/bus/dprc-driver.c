	...
	if (num_child_objects != 0) {
		...
		child_obj_desc_array =
		    devm_kzalloc(&mc_bus_dev->dev,
				 num_child_objects * sizeof(*child_obj_desc_array),
				 GFP_KERNEL);
...
static struct platform_device *fsl_mc_get_pdev(struct fsl_mc_device *mc_dev)
{
	...
	if (parent_dev->bus == &fsl_mc_bus_type) {  // it's a child device
		pdev = to_platform_device(parent_dev->parent);
	} else {  // it's the root DPRC
...
static int get_its_node(struct platform_device *pdev,
			struct its_node **out_its)
{
	struct device_node *pdev_of_node = pdev->dev.of_node;
	struct device_node *its_of_node = NULL;
	struct its_node *its;
	int error = -EINVAL;

	its_of_node = of_parse_phandle(pdev_of_node, "msi-parent", 0);
	if (its_of_node == NULL) {
		dev_err(&pdev->dev,
	...
	its = its_get_its_struct(its_of_node);
	if (its == NULL) {
		dev_err(&pdev->dev,
			"its_node not found %s\n",
			pdev_of_node->full_name);
		error = -ENODEV;
		goto out;
	}
...
static int dprc_probe(struct fsl_mc_device *mc_dev)
{
	...
	if (mc_bus->gic_its == NULL) {   /* check if interrupts are supported */
		struct fsl_mc *mc = dev_get_drvdata(fsl_mc_bus_type.dev_root->parent);

		/* get pointer to fsl-mc platform device associated with this dprc */
		pdev = fsl_mc_get_pdev(mc_dev);
		/* initialize gic_its struct pointer (needed for ITS API calls) */
		error = get_its_node(pdev, &mc_bus->gic_its);
		if (error < 0)
			dev_info(&mc_dev->dev, "fsl-mc: Warning: GIC ITS not found");

		/* init gic_supported field */
		mc = dev_get_drvdata(&pdev->dev);
