					...
					u32 *status)
{
	...
	if (ctrlpriv->virt_en == 1) {
		...
		caam_setbits32(&ctrl->deco_rsr, DECORSR_JR0);
#else
...
#if defined(CONFIG_ARM) || defined(CONFIG_ARM64)
	caam_setbits32(&ctrl->deco_rq, DECORR_RQD0ENABLE);
#else
	...
	*status = rd_reg32(&deco->op_status_hi) &
	...
	if (ctrlpriv->virt_en == 1)
#if defined(CONFIG_ARM) || defined(CONFIG_ARM64)
		caam_clrbits32(&ctrl->deco_rsr, DECORSR_JR0);
#else
...
#if defined(CONFIG_ARM) || defined(CONFIG_ARM64)
	caam_clrbits32(&ctrl->deco_rq, DECORR_RQD0ENABLE);
#else
...
static void kick_trng(struct device *dev, int ent_delay)
{
...
#if defined(CONFIG_ARM) || defined(CONFIG_ARM64)
	caam_setbits32(&r4tst->rtmctl, RTMCTL_PRGM);
#else
...
#if defined(CONFIG_ARM) || defined(CONFIG_ARM64)
		caam_clrbits32(&r4tst->rtmctl, RTMCTL_PRGM);
#else
	 ...
	 */
#if defined(CONFIG_ARM) || defined(CONFIG_ARM64)
	caam_setbits32(&val, RTMCTL_SAMP_MODE_RAW_ES_SC);
	/* put RNG4 into run mode */
	caam_clrbits32(&val, RTMCTL_PRGM);
#else
...
#if defined(CONFIG_ARM) || defined(CONFIG_ARM64)
		caam_setbits32(&ctrl->scfgr, SCFGR_RDBENABLE);
#else
...
static int caam_probe(struct platform_device *pdev)
{
	...
	if (ctrlpriv->virt_en == 1)
#if defined(CONFIG_ARM) || defined(CONFIG_ARM64)
		caam_setbits32(&ctrl->jrstart, JRSTART_JR0_START |
...
#if defined(CONFIG_ARM) || defined(CONFIG_ARM64)
	caam_setbits32(&qi->qi_control_lo, QICTL_STOP);
#else
