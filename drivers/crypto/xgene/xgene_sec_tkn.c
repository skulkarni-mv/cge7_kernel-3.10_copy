					 ...
					 unsigned char assoclen, int enc)
{
	...
	memcpy(&t[tl], (u8 *) iv_B0, 16);
	tl += 4;
	/* Insert adata length field from Token to hash */
	if (adata_length_length == 2) {
		t[tl++] = ((u32) * (u16 *) adata_length) << 8;
	} else {
		t[tl++] = *(u32 *) adata_length;
		t[tl++] = ((u32) * (u16 *) (adata_length + 4)) << 8;
	}
 ...
 */
inline unsigned short sec_tkn_set_dtls_out(unsigned int *t, unsigned int pl,
					   unsigned long long sa,
					   unsigned int *ctrl,
					   unsigned int padlen,
					   ...
					   int ivsize, int enc)
{
	...
	/* Insert fragment length to OUTPUT */
	sec_tkn_set32le(&t[tl++], 0x21D80002);
	sec_tkn_set32le(&t[tl++],
	...
	/* Create token header */
	sec_tkn_set_hdr(t, tlen, 0, 0, 1, 0, 0, 0, pl, 1, tl, sa);
					  ...
					  int enc)
{
	...
	sec_tkn_set32le(&t[tl++], ctrl[1]);
	/* Create token instruction - Process SSL header */
	/* Retrieve type + version to SPI result */
	sec_tkn_set32le(&t[tl++], 0x40900003);
	sec_tkn_set32le(&t[tl++], 0x42480004);	/* Insert SEQ HI to HASH */
	sec_tkn_set32le(&t[tl++], 0x42400004);	/* Insert SEQ LO to HASH */
	...
	/* Remove fragment length from INPUT */
	sec_tkn_set32le(&t[tl++], 0x40000002);
	
	/* For DES, 3DES, and AES, add IV */
	/* Insert the IV to Out */
	sec_tkn_set32le(&t[tl++], 0x42A00000 | ivsize);
	icv_pointer = pyl;
	...
	sec_tkn_set32le(&t[tl++], 0xE4561800 | ((seq_offset - 2) + 0x20));
...
/*
 * SSL/TLS Token Templates
 */
inline unsigned short sec_tkn_set_ssl_tls_out(unsigned int *t, unsigned int pl,
					      unsigned long long sa,
					      unsigned int *ctrl,
					      unsigned int padlen,
					      unsigned char seq_offset,
					      int icvsize, int arc4, int enc, 
					      int arc4_state_ptr,
					      ...
					      int ivsize)
{
	...
	/* Create token header */
	if (arc4) {
		sec_tkn_set_hdr(t, tlen, 0, 0, 1, 0, 0, 0, pl, 3, tl, sa);
	} else {
		sec_tkn_set_hdr(t, tlen, 0, 0, 1, 0, 0, 0, pl, 1, tl, sa);
	}
					     ...
					     int enc, int opcode, int ivsize)
{
	...
	/* Retrieve type + version to SPI result */
	sec_tkn_set32le(&t[tl++], 0x40900003);
	/* Insert SEQ HI to HASH */
	sec_tkn_set32le(&t[tl++], 0x22580004);
	/* Insert SEQ LO to HASH */
	sec_tkn_set32le(&t[tl++], 0x22500004);
	if (opcode == SA_OPCODE_TLS) {
		sec_tkn_set32le(&t[tl++], 0x22900003);
	} else {
	...
	/* Remove fragment length from INPUT */
	sec_tkn_set32le(&t[tl++], 0x40D80002);
