					...
					unsigned char *ipad_opad)
{
	...
	if (dl < 64) {
		...
		ipad += 16;
		sec_sa_setbe64((u64 *) ipad_opad, ipad, dl);
	}
	...
	if (dl < 64) {
		...
		opad += 16;
		sec_sa_setbe64((u64 *) (ipad_opad + dl), opad, dl);
	}
...
static int xgene_sec_setkey_aes_sha384_authenc(struct crypto_aead *cipher,
					       const unsigned char *key,
					       unsigned int keylen)
{
	return xgene_sec_setkey_cbc_hmac_authenc(cipher, key, keylen,
...
static int xgene_sec_setkey_aes_sha512_authenc(struct crypto_aead *cipher,
					       const unsigned char *key,
					       unsigned int keylen)
{
	return xgene_sec_setkey_cbc_hmac_authenc(cipher, key, keylen,
...
static int xgene_sec_setkey_3des_sha384_authenc(struct crypto_aead *cipher,
						const unsigned char *key,
						unsigned int keylen)
{
	return xgene_sec_setkey_cbc_hmac_authenc(cipher, key, keylen,
...
static int xgene_sec_setkey_3des_sha512_authenc(struct crypto_aead *cipher,
						const unsigned char *key,
						unsigned int keylen)
{
	return xgene_sec_setkey_cbc_hmac_authenc(cipher, key, keylen,
				    ...
				    unsigned int keylen)
{
	...
	rc = xgene_sec_create_sa_tkn_pool(session, SA_AES_CCM_MAX_LEN, 
					  SA_AES_CCM_LEN(keylen), 0, 
					  TKN_AES_CCM_MAX);
	if (rc != 0)
		return rc;
	...
	/* XCBC key 1 is swaped from crypto key */
	sa_hash = SA_DIGEST(session->sa->sa_ptr) + 8;
	for (;keylen > 0; key += 4, keylen -=4) 
		*sa_hash++ = swab32(*(unsigned int *)key);
...
static int xgene_sec_encrypt_aes_ccm(struct aead_request *req)
{
	...
	APMSEC_TXLOG("encrypt ccm %s nbytes %d", 
	...
	if (req->iv[0] != 1 && req->iv[0] != 3) {
		APMSEC_TXLOG("algorithm AES-CCM unsupported counter length %d",
			     req->iv[0] & 0x7);
		return -EINVAL;
	}
#endif
	...
	/* 
	 * Copy nonce IV and salt,
	 * L length (encoded) was in byte 0 of input IV vector
	 */
	memcpy(iv_B0, req->iv, 16);
	/* Calculate M lenth (encoded) */
	icvsize = crypto_aead_authsize(aead);
	iv_B0[0] |= (((icvsize - 2) / 2) << 3);
	/* Adata = 1 if AAD is not zero */
	if (req->assoclen)
		iv_B0[0] |= 0x40;
	/* Calculate Length (m) */
	...
	memcpy(SA_IV(session->sa->sa_ptr), req->iv, 16);
	...
	if (new_tkn) {
		rctx->tkn->input_tkn_len = sec_tkn_set_crypt_aes_ccm(
			(u32 *) in_tkn,	req->cryptlen,
			SA_PTR_HW(xgene_sec_encode2hwaddr(
				  SA_CONTEXT_PTR(session->sa->sa_hwptr))),
			(u32 *) session->sa->sa_ptr, icvsize, (u32 *) iv_B0,
			adata_length, adata_length_length, 
			adata, req->assoclen, 1);
		BUG_ON(rctx->tkn->input_tkn_len > session->tkn_input_len);
	} else {
		sec_tkn_update_crypt_aes_ccm(
			(u32 *) in_tkn,	req->cryptlen,
			SA_PTR_HW(xgene_sec_encode2hwaddr(
				  SA_CONTEXT_PTR(session->sa->sa_hwptr))),
			(u32 *) session->sa->sa_ptr, icvsize, (u32 *) iv_B0,
...
static int xgene_sec_decrypt_aes_ccm(struct aead_request *req)
{
	...
	if (req->assoclen != 0 && req->assoclen != 8 &&
	    req->assoclen != 12 && req->assoclen != 16) {
		APMSEC_RXLOG("algorithm AES-CCM unsupported adata length %d",
			     req->assoclen);
		return -EINVAL;
	}
	/* Support only counter field length of 2 and 4 bytes */
	if (req->iv[0] != 1 && req->iv[0] != 3) {
		APMSEC_RXLOG("algorithm AES-CCM unsupported counter length %d",
			     req->iv[0] & 0x7);
		return -EINVAL;
	}
#endif
	...
	if (rctx->tkn == NULL)
		return -ENOMEM;
	rctx->tkn->flags = 0;
	session->sa->sa_ptr->ToP = SA_TOP_DECRYPT_HASH_INBOUND;
	...
	/* 
	 * Decrypt format control info per RFC 3610 and NIST Special 
	 * Publication 800-38C
	 */
	/* Calculate M lenth (encoded) */
	icvsize = crypto_aead_authsize(aead);
	...
	if (req->assoclen)
		adata = kmalloc(req->assoclen, GFP_KERNEL);
	if (adata == NULL) {
		rc = -ENOMEM;
		goto err_dec_aes_ccm;
	}
	...
	if (new_tkn) {
		rctx->tkn->input_tkn_len = sec_tkn_set_crypt_aes_ccm(
			...
			SA_PTR_HW(xgene_sec_encode2hwaddr(
				  SA_CONTEXT_PTR(session->sa->sa_hwptr))),
			(u32 *) session->sa->sa_ptr, icvsize,
			(u32 *) iv_B0, adata_length, adata_length_length,
	...
	if (rc != 0)
		APMSEC_TXLOG("decrypt ccm failed error %d", rc);
 ...
 * DTLS Algorithm
 */
static int _xgene_sec_setkey_dtls(struct crypto_aead *cipher, const u8 *key,
				  ...
				  unsigned char ha)
{
	...
	*/
	...
	/* Load SPI and SEQ. Swab to prepare filling into Hash engine */
	*SA_SPI(session->sa->sa_ptr)	   = swab32(param->spi);
	*SA_SEQ(session->sa->sa_ptr)	   = swab32(param->seq_l);
	*(SA_SEQ(session->sa->sa_ptr) + 1) = swab32(param->seq_h);
...

{
	...
	if (IS_ERR(blk_tfm)) {
		printk("Failed to load helper transform for cbc(aes) error %ld",
		       PTR_ERR(blk_tfm));
		return -EINVAL;
	}
	blk_desc.tfm   = blk_tfm;
	...
	rc = crypto_blkcipher_setkey(blk_tfm, key, keylen);
	if (rc != 0) {
		printk("Failed to set helper transform key for ccb(aes) "
		       "error %d", rc);
		rc = -EINVAL;
		goto err;
	}
	...
	if (rc != 0) {
		printk("Failed to decrypt last 2 block error %d", rc);
		rc = -ENOMEM;
		goto err;
	}
...
static int _xgene_sec_encrypt_dtls(struct aead_request *req)
{
	...
	APMSEC_TXLOG("encrypt dtls %s nbytes %d", alg->cra_name, req->cryptlen);
	rctx->sa  = NULL;
	rctx->tkn = xgene_sec_tkn_get(session, &new_tkn);
	if (rctx->tkn == NULL)
		return -ENOMEM;
	...
	in_tkn  = TKN_CTX_INPUT_TKN(rctx->tkn);
	APMSEC_TXLOG("DTLS tkn 0x%p outtkn 0x%p intkn 0x%p", rctx->tkn,
		     TKN_CTX_RESULT_TKN(rctx->tkn), in_tkn);

	/* Outbound hash-encrypt operation */
	session->sa->sa_ptr->ToP = SA_TOP_HASH_ENCRYPT_OUTBOUND;
	...
	/* 
	 * DTLS packet format:
	 * Type  | Version | Epoch  | Seq number | Length of fragment | Fragment
	 * 1 byte| 2 byte  | 2 byte | 6 byte	 | 2 byte	      | 
	 * 		______________________________________________/	       \
	 * 			IV | Payload | MAC              | pading | L(pad)
	 *		block size |         | HMAC digest size |	 | 1 byte 
	 */
	/* Calculate padding length */
	pdl  = ALIGN((clen + icvsize + 1),
	...
	rctx->tkn->sa		= session->sa;
	rctx->tkn->context	= &req->base;
	rctx->tkn->dest_nbytes	= req->cryptlen + pdl + icvsize + 13 + 1 +
				  alg->cra_aead.ivsize;
	...
	if (rc != 0)
		dev_dbg(session->ctx->dev, "Encrypt DTLS failed error %d", rc);
...
static int _xgene_sec_decrypt_dtls(struct aead_request *req)
{
	...
	APMSEC_TXLOG("decrypt dtls %s nbytes %d", alg->cra_name, req->cryptlen);
	rctx->sa  = NULL;
	rctx->tkn = xgene_sec_tkn_get(session, &new_tkn);
	if (rctx->tkn == NULL)
		return -ENOMEM;
	...
	/* Inbound decrypt-hash operation */
	session->sa->sa_ptr->ToP = SA_TOP_DECRYPT_HASH_INBOUND;
	 ...
	 *		block size |         | HMAC digest size |	 | 1 byte 
	 */
	/* Decrypt the last 2 block of input data before putting to EIP96 */
	spage = kmalloc(req->cryptlen, GFP_ATOMIC);
	if (spage == NULL) 
		return -ENOMEM;
	nents = sg_nents(req->src);
        sg_copy_to_buffer(req->src, nents, spage, req->cryptlen);

	/* Frag len is located at Packet's 11t byte and 12th bye */
	memcpy(padlen, ((spage + req->cryptlen) - blocklen), blocklen);
	kfree(spage);

	xgene_sec_ssl_decrypt_pad_len((char *)SA_KEY(session->sa->sa_ptr),
				      16, blocklen, padlen, req->iv);
	/* Padding length */
	pdl = padlen[31] + 1;
	...
	rctx->tkn->input_tkn_len = sec_tkn_set_dtls_in(
		(u32 *) in_tkn,	req->cryptlen, pyl,
		SA_PTR_HW(xgene_sec_encode2hwaddr(
			  SA_CONTEXT_PTR(session->sa->sa_hwptr))),
		(u32 *) session->sa->sa_ptr, pdl,
		SA_SEQ_OFFSET(session->sa->sa_ptr), icvsize,
		session->sa->sa_ptr->hash_alg, alg->cra_aead.ivsize, 0);
	BUG_ON(rctx->tkn->input_tkn_len > session->tkn_input_len);
	rctx->tkn->sa		= session->sa;
	rctx->tkn->context	= &req->base;
	rctx->tkn->dest_nbytes	= req->cryptlen - (pdl + icvsize + 13 +
						   alg->cra_aead.ivsize);
	...
	if (rc != 0)
		dev_dbg(session->ctx->dev, "Decrypt DTLS  failed error %d", rc);
 ...
 * SSL, TLS algorithms
 */
static int _xgene_sec_setkey_ssl_tls(struct crypto_aead *cipher, const u8 *key,
				     unsigned int keylen, unsigned char ca,
				     unsigned char ha, int opcode)
{
	...
	if (opcode == SA_OPCODE_TLS) {
		...
		if (ha == SA_HASH_ALG_MD5) {
			...
			sec_sa_setbe(SA_DIGEST(session->sa->sa_ptr), key,
				     authkeylen);
		}
	}
					 ...
					 const u8 *key, unsigned int keylen)
{
	return _xgene_sec_setkey_ssl_tls(cipher, key, keylen,
					 SA_CRYPTO_ALG_RSVD,
					 SA_HASH_ALG_SHA1, SA_OPCODE_TLS);
}
...
static int _xgene_sec_encrypt_ssl_tls(struct aead_request *req, int opcode)
{
	...
	APMSEC_TXLOG("encrypt  ssl %s nbytes %d", alg->cra_name, req->cryptlen);
	rctx->sa  = NULL;
	rctx->tkn = xgene_sec_tkn_get(session, &new_tkn);
	if (rctx->tkn == NULL)
		return -ENOMEM;
	...
	in_tkn  = TKN_CTX_INPUT_TKN(rctx->tkn);
	APMSEC_TXLOG("SSL tkn 0x%p outtkn 0x%p intkn 0x%p", rctx->tkn,
		      TKN_CTX_RESULT_TKN(rctx->tkn), in_tkn);
	 ...
	 * Type  | Version | Length of fragment | Fragment
	...
	rctx->tkn->input_tkn_len = sec_tkn_set_ssl_tls_out(
		...
		(u32 *) session->sa->sa_ptr,
		pdl, SA_SEQ_OFFSET(session->sa->sa_ptr), icvsize,
		0, 1, 0, 0, opcode, alg->cra_aead.ivsize);
	BUG_ON(rctx->tkn->input_tkn_len > session->tkn_input_len);
	rctx->tkn->sa	   = session->sa;
	rctx->tkn->context = &req->base;
	...
	if (rc != 0)
		dev_dbg(session->ctx->dev, "Encrypt SSL/TLS failed error %d",
			rc);
...
static int _xgene_sec_decrypt_ssl_tls(struct aead_request *req, int opcode)
{
	...
	APMSEC_TXLOG("decrypt  ssl %s nbytes %d", alg->cra_name, req->cryptlen);
	rctx->sa  = NULL;
	rctx->tkn = xgene_sec_tkn_get(session, &new_tkn);
	if (rctx->tkn == NULL)
		return -ENOMEM;
	...
	rctx->tkn->input_tkn_len = sec_tkn_set_ssl_tls_in(
		...
		SA_PTR_HW(xgene_sec_encode2hwaddr(
			SA_CONTEXT_PTR(session->sa->sa_hwptr))),
		(u32 *) session->sa->sa_ptr, pdl, 
		SA_SEQ_OFFSET(session->sa_ib->sa_ptr), icvsize,
		session->sa->sa_ptr->hash_alg,
	...
	/* Calculate output length */
	if (opcode == SA_OPCODE_SSL)
		rctx->tkn->dest_nbytes = req->cryptlen - pdl - icvsize - 5;
	else {
		rctx->tkn->dest_nbytes = req->cryptlen - pdl - icvsize - 5 -
					 alg->cra_aead.ivsize;
	}
	...
	if (rc != 0)
		dev_dbg(session->ctx->dev, "Decrypt SSL/TLS failed error %d",
			rc);
...
{
	return _xgene_sec_encrypt_ssl_tls(req, SA_OPCODE_TLS);
}

static int xgene_sec_decrypt_tls1_1(struct aead_request *req)
{
	return _xgene_sec_decrypt_ssl_tls(req, SA_OPCODE_TLS);
}
...
struct xgene_sec_alg xgene_sec_alg_tlb[] = {
	...
	{.type = CRYPTO_ALG_TYPE_AHASH, .u.hash = {
	 ...
	 .cra_driver_name 	= "hmac-sha384-xgene",
	 .cra_flags 		= CRYPTO_ALG_TYPE_AHASH | CRYPTO_ALG_ASYNC,
	 .cra_blocksize 	= 128,	/* SHA384-HMAC block size is 512-bits */
	 .cra_ctxsize 		= sizeof(struct xgene_sec_session_ctx),
	 .cra_alignmask 	= 0,	/* Hardware requires 16 bytes aligned */
	...
	{.type = CRYPTO_ALG_TYPE_AHASH, .u.hash = {
	 ...
	 .cra_driver_name 	= "hmac-sha512-xgene",
	 .cra_flags 		= CRYPTO_ALG_TYPE_AHASH | CRYPTO_ALG_ASYNC,
	 .cra_blocksize 	= 128,	/* SHA512-HMAC block size is 1024-bits*/
	 .cra_ctxsize 		= sizeof(struct xgene_sec_session_ctx),
	 .cra_alignmask 	= 0,	/* Hardware requires 16 bytes aligned */
	...
	{.type = CRYPTO_ALG_TYPE_ABLKCIPHER, .u.cipher = {
	 ...
	 /*
	  * FIXME: This algorithm is NOT passed testmgr. Will be fix later
	  */
#if 0
	{.type = CRYPTO_ALG_TYPE_ABLKCIPHER, .u.cipher = {
	 .cra_name 		= "ecb(des)",
	...
	.cra_driver_name	= "authenc-hmac-sha384-cbc-aes-xgene",
	.cra_flags		= CRYPTO_ALG_TYPE_AEAD | CRYPTO_ALG_ASYNC,
	.cra_blocksize		= 16,	/* 128-bits block */
	.cra_ctxsize		= sizeof(struct xgene_sec_session_ctx),
	.cra_alignmask		= 0xF,	/* Hardware requires 16 bytes aligned */
	...
	.setkey			= xgene_sec_setkey_aes_sha384_authenc,
	.setauthsize		= xgene_sec_setauthsize_aes,
	.encrypt		= xgene_sec_encrypt_cbc_authenc,
	.decrypt		= xgene_sec_decrypt_cbc_authenc,
	.givencrypt		= xgene_sec_givencrypt_cbc_authenc,
	...
	.cra_driver_name	= "authenc-hmac-sha512-cbc-aes-xgene",
	.cra_flags		= CRYPTO_ALG_TYPE_AEAD | CRYPTO_ALG_ASYNC,
	.cra_blocksize		= 16,	/* 128-bits block */
	.cra_ctxsize		= sizeof(struct xgene_sec_session_ctx),
	.cra_alignmask		= 0xF,	/* Hardware requires 16 bytes aligned */
	...
	.setkey			= xgene_sec_setkey_aes_sha512_authenc,
	.setauthsize		= xgene_sec_setauthsize_aes,
	.encrypt		= xgene_sec_encrypt_cbc_authenc,
	.decrypt		= xgene_sec_decrypt_cbc_authenc,
	.givencrypt		= xgene_sec_givencrypt_cbc_authenc,
	...
	.cra_driver_name	= "authenc-hmac-sha384-cbc-des3-xgene",
	.cra_flags		= CRYPTO_ALG_TYPE_AEAD | CRYPTO_ALG_ASYNC,
	.cra_blocksize		= 16,	/* 128-bits block */
	.cra_ctxsize		= sizeof(struct xgene_sec_session_ctx),
	.cra_alignmask		= 0xF,	/* Hardware requires 16 bytes aligned */
	...
	.ivsize			= 8,	/* IV size is 8 bytes */
	.maxauthsize		= 48,	/* Max auth data size in bytes */
	.setkey			= xgene_sec_setkey_3des_sha384_authenc,
	.setauthsize		= xgene_sec_setauthsize_aes,
	.encrypt		= xgene_sec_encrypt_cbc_authenc,
	.decrypt		= xgene_sec_decrypt_cbc_authenc,
	.givencrypt		= xgene_sec_givencrypt_cbc_authenc,
	...
	.cra_driver_name	= "authenc-hmac-sha512-cbc-des3-xgene",
	.cra_flags		= CRYPTO_ALG_TYPE_AEAD | CRYPTO_ALG_ASYNC,
	.cra_blocksize		= 16,	/* 128-bits block */
	.cra_ctxsize		= sizeof(struct xgene_sec_session_ctx),
	.cra_alignmask		= 0xF,	/* Hardware requires 16 bytes aligned */
	...
	.ivsize			= 8,	/* IV size is 8 bytes */
	.maxauthsize		= 64,	/* Max auth data size in bytes */
	.setkey			= xgene_sec_setkey_3des_sha512_authenc,
	.setauthsize		= xgene_sec_setauthsize_aes,
	.encrypt		= xgene_sec_encrypt_cbc_authenc,
	.decrypt		= xgene_sec_decrypt_cbc_authenc,
	.givencrypt		= xgene_sec_givencrypt_cbc_authenc,
	...
	.cra_driver_name 	= "dtls-aes-sha1-xgene",
	.cra_flags 		= CRYPTO_ALG_TYPE_AEAD | CRYPTO_ALG_ASYNC,
	...
	.cra_driver_name	= "ssl-aes-sha1-xgene",
	.cra_flags 		= CRYPTO_ALG_TYPE_AEAD | CRYPTO_ALG_ASYNC,
	.cra_blocksize 		= 16,	/* 128-bits block */
	.cra_ctxsize 		= sizeof(struct xgene_sec_session_ctx),
	.cra_alignmask 		= 0xF,
