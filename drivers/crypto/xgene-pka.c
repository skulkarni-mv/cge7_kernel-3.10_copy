 ...
 */
static int apm_pka_probe(struct platform_device *pdev)
{
	struct resource *res;
	int rc;
	...
	if (!res) {
		dev_err(&pdev->dev, "PKA Error: failed to get address to resource\n");
		return -ENODEV;
	}
	memcpy(&pka_dev.csr_res, res, sizeof(struct resource));
	...
	if (pka_dev.ctx.csr == NULL) {
		dev_err(&pdev->dev, "PKA Error: failed to ioremap 0x%02X_%08X size %d\n",
	...
	pka_dev.ctx.irq = platform_get_irq(pdev, 0);
	if (pka_dev.ctx.irq > 0) {
		rc = request_irq(pka_dev.ctx.irq, apm_pka_irq_handler,
				 0, "PKA", NULL);
		if (rc) {
                	dev_err(&pdev->dev, "PKA Error: failed to register PKA Interrupt Handler\n");
			goto err;
		}
	...
	rc = pka_clk_init(&pdev->dev);
        if (rc)
                goto err;

	rc = apm_pka_hw_init();
	if (rc < 0) {
		dev_err(&pdev->dev, "PKA Error: Failed to initialize PKA HW\n");
		goto err;
	}
