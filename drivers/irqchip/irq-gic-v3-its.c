			    ...
			    bool force)
{
	...
	if (IS_ENABLED(CONFIG_THUNDERX_PASS1_ERRATA_23144) && is_multi_its) {
		...
		node =  its_dev->its_vec_ne[vec_nr]->node_id;

		if (!cpumask_intersects(mask_val, cpumask_of_node(node))) {
			its_mask_irq(d);
			its_dev->its =
				get_its_other_node(its_dev->its_vec_ne[vec_nr]);
			its_dev->collection = &its_dev->its->collections[cpu];
			its_mask_irq(d);
			its_dev->its_vec_ne[vec_nr] = its_dev->its;
			...
			its_dev->its = its_dev->its_vec_ne[vec_nr];
			its_dev->collection = &its_dev->its->collections[cpu];
		}
	}
	...
	if (IS_ENABLED(CONFIG_PCI_MSI) && d->msi_desc)
		...
		id = d->hwirq - its_dev->lpi_base;
	if (cpus_have_cap(ARM64_WORKAROUND_CAVIUM_21306))
					    ...
					    int nvecs)
{
	...
	if (IS_ENABLED(CONFIG_THUNDERX_PASS1_ERRATA_23144) && is_multi_its) {
		...
		dev->its_vec_ne = kzalloc((sizeof(*its) * nvecs), GFP_KERNEL);
		if (!dev->its_vec_ne) {
			kfree(dev);
			kfree(itt);
			kfree(lpi_map);
...
static u32 its_dflt_pci_requester_id(struct pci_dev *pdev, u16 alias)
{
	...
	return of_msi_map_rid(&pdev->dev, NULL, rid);
}
...
{
	return its_pci_requester_id(dev, 0);
}
EXPORT_SYMBOL(pci_requester_id);

/*
 *  For a given ICID (ITS device id) and vector number, create
 *  the its device if needed, and allocate an irq
 *
 *  Returns irq and MSI addr. (MSI data is vec_nr)
 */
int its_fsl_mc_setup_irq(struct its_node *its,
			 u32 dev_id,
			 u32 vec_nr,
			 int nvec,
			 unsigned int *irq,
			 u64 *msi_addr)
{
	...
	error = its_alloc_device_irq(its_dev, vec_nr, &hwirq, irq);
	if (error)
		return error;

	irq_set_handler_data(*irq, its_dev);

	*msi_addr = its->phys_base + GITS_TRANSLATER;

	return 0;
}

void its_fsl_mc_teardown_irq(u32 vec_nr, unsigned int irq)
{
	...
	its_send_discard(its_dev, vec_nr);
...
static int its_msi_setup_irq(struct msi_controller *chip,
			     struct pci_dev *pdev,
...
static void its_msi_teardown_irq(struct msi_controller *chip, unsigned int irq)
{
	struct irq_data *d = irq_get_irq_data(irq);
	struct its_device *its_dev = irq_data_get_irq_handler_data(d);
...
struct its_node *its_get_its_struct(struct device_node *of_node)
{
	struct list_head *entry;
	...
	list_for_each(entry, &its_nodes) {
		...
		if (tmp->msi_chip.of_node != of_node)
			continue;
