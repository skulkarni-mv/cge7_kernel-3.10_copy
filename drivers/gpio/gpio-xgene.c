...
struct xgene_gpio {
	...
	u32			mux_val[XGENE_MAX_GPIO_BANKS];
#endif
};
...
{
	struct xgene_gpio *chip = to_xgene_gpio(gc);
	unsigned long flags, bank_offset, mux_offset;
	u32 dirval, bit_offset, muxval;

	bank_offset = GPIO_SET_DR_OFFSET + GPIO_BANK_OFFSET(offset);
	mux_offset  = GPIO_MUX_OFFSET + GPIO_BANK_OFFSET(offset);
	bit_offset = GPIO_BIT_OFFSET(offset);
	...
	muxval = ioread32(chip->base + mux_offset);
	muxval |= 0x3 << (2 * bit_offset);
	iowrite32(muxval, chip->base + mux_offset);
...
static int xgene_gpio_resume(struct device *dev)
{
	...
	for (bank = 0; bank < XGENE_MAX_GPIO_BANKS; bank++) {
		...
		iowrite32(gpio->mux_val[bank], gpio->base + mux_offset);
	}
	return 0;
}
