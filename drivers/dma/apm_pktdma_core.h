...
/* Queue descriptor size is 64K. Each descriptor is 32B or 64B. With 64B,
   the number of descriptor is 1K (or 2K for 32B).
*/
#define PKTDMA_MAX_OPS		(1024 * 2)
#define PKTDMA_MAX_OPS_EXT	(512)
#define PKTDMA_MAX_OPS_INT	(PKTDMA_MAX_OPS - PKTDMA_MAX_OPS_EXT)
...
/** HW constructs */
#define APM_PKTDMA_OP_CACHE_MAX 2048
...
#define PKTDMA_MAX_XOR			5

#define PKDTMA_XOR_CHAN			0
...
#define PKTDMA_OP_SIGN		0x00504B54
...
struct iodma_op_state {
	...
	u8 major_opcode;
	u8 fby_gen;
	u8 chk_slot;		/* Inflight slot check activated */
	u8 cos;
	struct pktdma_chan_qid *chqid;
...
struct pktdma_chan_qid {
	...
	int tx_max_slot[APM_PKTDMA_NUM_COS];	  /* Slot for external module */
	atomic_t tx_inflight[APM_PKTDMA_NUM_COS]; /* Inflight for external module */
	int slot_rsvd;		/* Number slot reserved for non-PPC */
};
...
struct pktdma_q_info {
	...
	struct pktdma_queue_ids queues[MAX_CORES];
...
/**
 * @brief Read PktDMA Diagnostics CSR
 * @param   offset Register offset
 * @param   data A pointer to store the data
 * @return  0 on success. Non-0 on failure.
 */
...
/**
 * @brief Write PktDMA Diagnostics CSR
 * @param   offset Register offset
 * @param   data Value to write
 * @return  0 on success. Non-0 on failure.
 */
...
/**
 * @brief Determine if cache is coherent
