/*
 * spi-xgene.c - AppliedMicro X-Gene SPI Driver (Master Only).
				...
				struct spi_transfer *t2)
{
	...
	unsigned long flags;
	u8 *tx, *rx;
	u32 old_ctrl0, old_ctrl1, old_ser, sr;
	u32 tx_pos, tx_cnt, rx_pos, rx_cnt, rx_len, rx_tmp;
	u32 flash_addr_next;
	...
	do {
		if (rx_cnt > XGENE_MAX_FIFO_DEPTH)
			rx_len = XGENE_MAX_FIFO_DEPTH;
		else
			rx_len = rx_cnt;
		...
		xgene_spi_writel(&regs->ctrlr1, (rx_tmp) ? (rx_tmp - 1) : 0);
		...
		if (rx_cnt > XGENE_MAX_FIFO_DEPTH) {
			flash_addr_next = be32_to_cpu(*(u32 *)(&tx[1])) + XGENE_MAX_FIFO_DEPTH;
			*(u32 *)(&tx[1]) = cpu_to_be32(flash_addr_next);

			tx_pos = 0;
			tx_cnt = t1->len;

			rx_cnt -= XGENE_MAX_FIFO_DEPTH;
			rx_len = 0;
		}
