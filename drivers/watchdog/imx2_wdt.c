...
static inline u16 imx2_read_wdt_reg(const volatile void __iomem *addr)
{
	u16 val;
	val = __raw_readw(addr);
#ifdef CONFIG_IMX2_WDT_BYTE_SWAP
	val = swab16(val);
#endif
	return val;
}

static inline void imx2_write_wdt_reg(u16 val, volatile void __iomem *addr)
{
#ifdef CONFIG_IMX2_WDT_BYTE_SWAP
	val = swab16(val);
#endif
				...
				void *cmd)
{
	...
	/* Assert SRS signal */
	imx2_write_wdt_reg(wcr_enable, imx2_wdt.base + IMX2_WDT_WCR);
	/*
...
{
	u16 val = imx2_read_wdt_reg(imx2_wdt.base + IMX2_WDT_WCR);
	...
	imx2_write_wdt_reg(val, imx2_wdt.base + IMX2_WDT_WCR);
...
{
	imx2_write_wdt_reg(IMX2_WDT_SEQ1, imx2_wdt.base + IMX2_WDT_WSR);
	imx2_write_wdt_reg(IMX2_WDT_SEQ2, imx2_wdt.base + IMX2_WDT_WSR);
}
							...
							unsigned long arg)
{
	...
	case WDIOC_GETBOOTSTATUS:
		val = imx2_read_wdt_reg(imx2_wdt.base + IMX2_WDT_WRSR);
...
static int __init imx2_wdt_probe(struct platform_device *pdev)
{
	...
	ret = register_restart_handler(&imx2_wdt.restart_handler);
	if (ret)
...
static int __exit imx2_wdt_remove(struct platform_device *pdev)
{
	...
	unregister_restart_handler(&imx2_wdt.restart_handler);
