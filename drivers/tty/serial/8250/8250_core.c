...
static void brcm_dw8250_force_idle(struct uart_port *p)
{
...
{
	offset = offset << p->regshift;
#if IS_ENABLED(CONFIG_APM86xxx)
        out_be32((void *) (p->membase + offset), value);
#else
...
static unsigned int mem32_serial_in(struct uart_port *p, int offset)
{
	offset = offset << p->regshift;
#if IS_ENABLED(CONFIG_APM86xxx)
        return in_be32((void *) (p->membase + offset));
#else
...
int serial8250_handle_irq(struct uart_port *port, unsigned int iir)
{
	...
	/*
	 * There are ways to get some buggy UARTs into a state where
	 * they are asserting UART_IIR_RX_TIMEOUT but there is no actual
	 * data available.  If we see such a case then we'll do a bogus
	 * read of the Receive FIFO.  If we don't do this then the 
	 * "RX TIMEOUT" interrupt will fire forever.
	 *
	 * This problem has only been observed so far when not in DMA mode
	 * so we limit the workaround only to non-DMA mode.
	 *
	 * Ripped off from the dw8250 driver.
	 */
