...
int apm_enet_get_pb_cnt(struct apm_data_priv *priv, int pbn)
{
	...
	if (pbn <= 7) {
		apm_enet_rd32(priv, BLOCK_ETH_QMI_SLAVE,
				ENET_STSSSQMIFPBUFFER1_ADDR, &val);
	} else {
		apm_enet_rd32(priv, BLOCK_ETH_QMI_SLAVE,
				ENET_STSSSQMIFPBUFFER2_ADDR, &val);
		pbn -= 8;
	}
	return (val >> (pbn * 3)) & 0x7;
}
				...
				u32 addroffset, u32 last, u32 *msg)
{
	...
	*msg = 0;
	...
	apm_enet_wr32(priv, BLOCK_ETH_QMI_SLAVE,
...
#if !defined(CONFIG_APM862xx) && !defined(ENET_LINK_POLL)
int apm_enet_enable_link_intr(struct apm_data_priv *priv, int enable)
{
	u32 val;
	int port_idx = priv->port % 2;
	int phy_addr = priv->phy_addr;

	ENET_DEBUG("%s link scanning for port %d\n",
		enable ? "Enable" : "Disable", port_idx);

	/* Program the PHY address for this port */
	apm_enet_rd32(priv, BLOCK_ETH_GBL, CFG_PHYID_ADDR, &val);
	val &= ~(0xF << (port_idx * 8));
	val |= (phy_addr << (port_idx * 8));
	apm_enet_wr32(priv, BLOCK_ETH_GBL, CFG_PHYID_ADDR, val);

	/* Enable auto-incr for scanning */
	apm_emac_read32(priv, BLOCK_ETH_MAC, MII_MGMT_CONFIG_ADDR, &val);
	...
	/* Enable interrupt */
	apm_enet_rd32(priv, BLOCK_ETH_GBL, LINK_AGGR_INTRMASK_ADDR, &val);
	if (enable)
		val &= ~(0x0001 << port_idx);
	else
		val |= (0x0001 << port_idx);
	apm_enet_wr32(priv, BLOCK_ETH_GBL, LINK_AGGR_INTRMASK_ADDR, val);
