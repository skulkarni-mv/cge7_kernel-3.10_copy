...
int apm_cle_avl_add(u32 port, u32 *node_value, u32 *node_index)
{
	...
	for (i = 0; i < REGS_PER_AVL_ADD; i++) {
		rc = apm_gbl_cle_wr32(cid, (DATA_RAM15_ADDR - (i * 4)),
					*(node_value + i));
	}
#else
	/* Write the Node data [0:319] in the dataram 6-15 register */
	for (i = 0; i < REGS_PER_AVL_ADD; i++) {
		rc = apm_gbl_cle_wr32(cid, (DATA_RAM6_ADDR + (i * 4)),
					*(node_value + i));
	}
#endif
...
#else
	/* Write the search string [0:255] in the dataram 6-13 register */
	for (i = 0; i < REGS_PER_AVL_DEL; i++) {
		rc = apm_gbl_cle_wr32(cid, (DATA_RAM6_ADDR + (i * 4)),
					*(search_string + i));
	}
...
int apm_cle_avl_rd(u32 port, u32 *node_value, u32 node_index)
{
	...
	if (counter_for_timeout > APM_CLE_ITER) {
		...
		for (i = 0; i < REGS_PER_AVL_NODE; i++) {
			rc |= apm_gbl_cle_rd32(cid, (DATA_RAM15_ADDR - (i * 4)),
						(node_value + i));
		}
	}
