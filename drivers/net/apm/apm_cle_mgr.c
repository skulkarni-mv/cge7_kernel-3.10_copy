...
/* ptree nodes for APM QoS should be at the tail of ptree area.
 * APM QoS deals with multiple commit to patricia ptree. It should know
 * last (non APM QoS) activity to allocate node and dbptr.
 * It helps to inhibit overriding former config (e.g LRO, IFO, INO ...)
 * */
static struct apm_cle_mgr_saved_stat saved_stat[MAX_CLE_ENGINE] = {{0}};
#endif
...
#ifdef ROLLBACK_MANAGER
/* APM QoS deals with multiple commit to patricia ptree.
 * Reset all settings occupied by previous commit.
 * */
int apm_cle_mgr_init_ext(int cid)
{
	int i;

	BUG_ON(cid < 0 || cid >= MAX_CLE_ENGINE);

	if (!saved_stat[cid].us_action_trigged) {
		saved_stat[cid].us_action_trigged = 1;
		saved_stat[cid].node_alloc_index = node_alloc_index[cid];
		saved_stat[cid].dbptr_alloc_index = dbptr_alloc_index[cid];
	}
	node_alloc_index[cid] = saved_stat[cid].node_alloc_index;
	dbptr_alloc_index[cid] = saved_stat[cid].dbptr_alloc_index;

	for (i = node_alloc_index[cid] ; i < MAX_NODE_ENTRIES ; i++ ) {
		SET_BITMASK(i, node_bitmask[cid], 0);
	}
	for (i = dbptr_alloc_index[cid] ; i < MAX_DB_ENTRIES ; i++ ) {
		SET_BITMASK(i, dbptr_bitmask[cid], 0);
	}
...
#ifdef ROLLBACK_MANAGER
/* APM QoS deals with multiple commit to patricia ptree.
 * Rollback patricia tree to the previous state of last commit. Preparing
 * for a new commit
 * */
int apm_ptree_rollback(u8 port,
	...
	if (ptree_config->start_node_ptr > node_alloc_index[cid]) {
		return APM_RC_ERROR;
	}
	for (i = ptree_config->start_node_ptr; i < node_alloc_index[cid]; i++) {
		if ((rc = apm_clear_ptree_node_type(cid, i)) != APM_RC_OK) {
			PCLS_ERR("Patricia Tree Node-Clear error %d \n", rc);
			return rc;
		}
		PCLS_PRNT("%s: Cleared node index %d\n", __func__, i);
	}

	/* Rollback node_alloc_index */
	node_alloc_index[cid] = ptree_config->start_node_ptr;
	/* Do not rollback dbptr_alloc_index */
#if 0
	dbptr_alloc_index[cid] = ptree_config->start_dbptr;
#endif

	PCLS_PRNT("%s: Restore node_alloc_index from %d to %d\n", __func__,
			saved_stat[cid].node_alloc_index, node_alloc_index[cid]);
	PCLS_PRNT("%s: Restore dbptr_alloc_index from %d to %d\n", __func__,
			saved_stat[cid].dbptr_alloc_index, dbptr_alloc_index[cid]);
	apm_cle_mgr_init_ext(cid);
