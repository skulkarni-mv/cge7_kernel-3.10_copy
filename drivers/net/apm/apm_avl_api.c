...
int apm_cle_avl_add_node(u32 port, struct avl_node *node)
{
	...
	for (i = 0; i < AVL_MAX_SEARCH_STRING; i++)
		search_key[i] = CPU_TO_LE32(node->search_key[i]);
	...
	node_value[0] = ((u32)(node->result_pointer
				...
				(search_key[0] << 13);
	for (i = 1; i < 8; i++) {
		node_value[i] = (search_key[i - 1] >> 19) |
			(search_key[i] << 13) ;
	}
	node_value[8] = (search_key[7] >> 19);
#else
	for (i = 0; i < AVL_MAX_SEARCH_STRING; i++)
		node_value[i] = node->search_key[i];

	node_value[8] = (node->priority & AVL_PRI_MASK);
	node_value[9] = (node->result_pointer & AVL_DB_PTR_MASK);
#endif
...
int apm_cle_avl_del_node(u32 port, struct avl_node *node)
{
	...
	rc = apm_cle_avl_del(port, search_key, &index);
#else
	rc = apm_cle_avl_del(port, node->search_key, &index);
#endif
...
int apm_cle_avl_search_node(u32 port, struct avl_node *node)
{
	...
	rc = apm_cle_avl_search(port, search_key, &node->index,
			&node->result_pointer);
#else
	rc = apm_cle_avl_search(port, node->search_key, &node->index,
			&node->result_pointer);
#endif
...
int apm_cle_avl_read_node(u32 port, struct avl_node *node)
{
	...
	for (i = 0; i < AVL_MAX_SEARCH_STRING; i++)
#ifdef CONFIG_APM862xx
		node->search_key[i] = LE32_TO_CPU(search_key[i]);
#else
