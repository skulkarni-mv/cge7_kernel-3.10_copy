...
#ifndef __APM_CLE_CFG_H_
#define __APM_CLE_CFG_H_
...
enum apm_sys_macaddr_index {
	ETHERNET_MACADDR = 0,	/* EthernetN MACAddr */
...
/**
 * @brief   This function programs given search key into Search Engines AVL tree.
 * @param   port_id - GE Port number (0)
 * @key     Search Key to be programmed.
 * @return  APM_RC_OK - success or APM_RC_ERROR - failure
 */
int apm_preclass_add_avl_entry(u32 port, u8 *key);

/**
 * @brief   This function delets given search key from Search Engines AVL tree.
 * @param   port_id - GE Port number (0)
 * @key     Search Key to be deleted.
 * @return  APM_RC_OK - success or APM_RC_ERROR - failure
 */
int apm_preclass_delete_avl_entry(u32 port, u8 *key);

#endif /* __APM_CLE_CFG_H_ */
