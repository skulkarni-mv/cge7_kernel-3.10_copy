				...
				int no_of_buffs, int qm_blk, int hdr_rsvd)
{
	...
	msg.BufDataLen = apm_qm_encode_bufdatalen(2048);	/* Set buffer size */
	msg.FPQNum = queue_id;
...
int apm_enet_rx_frame(struct apm_qm_msg_desc *rx_msg_desc)
{
	...
	u32 data_len = apm_qm_decode_datalen(msg16->BufDataLen) - 4;
	...
	*/
	if (unlikely(LErr && LErr != 7)) {
		/* Parse error */
		priv_dev->estats.rx_hw_errors++;
		if (apm_enet_parse_error(LErr, HC, rx_msg_desc->qid)) {
...
{
	u16 BufDataLen = apm_qm_encode_bufdatalen(2048);
	struct mb_qm_msg_desc fp_msg_desc;
	struct mb_qm_msg16 msg;
	char *tmp_ptr;
	phys_addr_t phy_addr;
