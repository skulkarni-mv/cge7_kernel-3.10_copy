...
int apm_enet_ioctl(struct net_device *ndev, struct ifreq *rq, int cmd)
{
	...
	case SIOREGACCESS:
	{
		...
		ENET_DEBUG("cmd id: %d block_id: %d reg_offset: 0x%08x "
	...
	case SIOCENETINFO:
	{
		...
		info_ops = &ioc->ioctl_ops.info_ops;

		info_ops->signature = AMCC_ENET_SIGNATURE;
		info_ops->port = pdev->port_id;
		break;
	}
	case SIOCIPAUSETHR:
	...
	case SIOCPRECLASSHALT:
		if ((rc = apm_preclass_start_stop(find_port(rq->ifr_name), 0))
		    != APM_RC_OK) {
	...
	case SIOCSCLEDBPTR:
	{
		...
		port = ioc->ioctl_ops.ptree_cfg.port;
		if ((rc = apm_set_cle_dbptr(port, dbptr)) != APM_RC_OK) {
...
end_ptree_alloc:
	...
	case SIOCPTREEFREE:
	{
		...
		apm_cle_mgr_init_ext(PID2CID[port]); /* brute-force to clear */
		rc = apm_del_ptree_config(port, apm_ptree_id);
		if (rc != APM_RC_OK) {
			ENET_DEBUG_ERR("Del Patricia Tree %s Configuration "
				"error for port %d\n", apm_ptree_id, port);
			rc = -EFAULT;
			/* goto end_ptree_alloc; */
		}
	...
	case SIOCPTREEROLLBACK:
	{
		...
		port = alloc_cfg->port;
		copy_from_user(&ptree_config,
					ioc->ioctl_ops.ptree_cfg.ptree_config,
					sizeof(struct apm_ptree_config));

		rc = apm_ptree_rollback(port, &ptree_config);
		if (rc != APM_RC_OK) {
			ENET_DEBUG_ERR("Rollback Patricia Tree Configuration "
				"error for port %d\n", port);
			rc = -EFAULT;
		}
		break;
	...
	case SIOCCLEAVLINIT:
		if ((rc = apm_cle_avl_init(port, ioc->ioctl_ops.key_size))
		    != APM_RC_OK) {
	...
	case SIOCCLEAVLADD:
		...
		if ((rc = apm_cle_avl_add_node(port, avl_node_cfg)) != APM_RC_OK) {
	...
	case SIOCCLEAVLDEL:
		avl_node_cfg = &ioc->ioctl_ops.avl_node_cfg;
		if ((rc = apm_cle_avl_del_node(port, avl_node_cfg)) != APM_RC_OK) {
	...
	case SIOCCLEAVLSRCH:
		avl_node_cfg = &ioc->ioctl_ops.avl_node_cfg;
		if ((rc = apm_cle_avl_search_node(port, avl_node_cfg)) !=
				APM_RC_OK) {
	...
	case SIOCCLEAVLSTATUS:
	{
		...
		if ((rc = apm_cle_avl_get_status(port, avl_status)) != APM_RC_OK) {
...
end_avl_alloc:
	...
	case SIOCSQMMSG16:
	{
		...
		pdev->in_poll_rx_msg_desc.is_msg16 = qm_msg16;
		break;
	}
#if defined(CONFIG_APM_ENET_LRO)
	case SIOLROENABLE: /* Enable LRO */
		apm_enet_lro_enable(pdev, ioc->ioctl_ops.lro_enable);
		break;
	case SIOLROTIMEOUT:
		apm_enet_lro_set_timeout(pdev,
					ioc->ioctl_ops.lro_timeoutms);
		break;
	case SIOLROBYTECNT:
		apm_enet_lro_set_maxbytecnt(pdev,
					ioc->ioctl_ops.lro_maxbytecnt);
		break;
	case SIOLROCTX:
	{
		u32 lro_ctx[MAX_LRO_PARAMS];
		u32 *lro_ctx_user = ioc->ioctl_ops.lro_ctx;

		if (lro_ctx_user == NULL) {
			rc = -EINVAL;
			goto err;
		}

		lro_ctx[LRO_SEGMENT_WQID] = pdev->lro.qm_sys.seg_rx_qid;
		lro_ctx[LRO_SEGMENT_FQPBN] = pdev->lro.qm_src.seg_fp_pbn;

		if (copy_to_user(lro_ctx_user, lro_ctx, sizeof(lro_ctx))) {
			rc = -EFAULT;
			goto err;
		}
...
#endif
#if defined(CONFIG_APM_ENET_QOS)
	/* Enable QoS */
	case SIOCQOSENABLE:
		apm_enet_qos_enable(pdev,ioc->ioctl_ops.qos_enable);
		break;
	/* Disable Mac Rx */
	case SIOCQOSRXDISABLE:
		apm_gmac_rx_disable(priv);
		break;
	/* Enable Mac Rx */
	case SIOCQOSRXENABLE:
		apm_gmac_rx_enable(priv);
		break;
	case SIOCGQOSCONFIG:
	{
		struct apm_qos_param *qos_cfg;
		qos_cfg = &ioc->ioctl_ops.qos_cfg;
		if (qos_cfg == NULL) {
			rc = -EINVAL;
			goto err;
		}
		/* Copy required data  to user */
		memcpy(qos_cfg, &pdev->qos.qos_cfg,
			sizeof(struct apm_qos_param));
		rc = APM_RC_OK;
		break;
	}
#endif
	...
	case SIOSLIMPROIPFWENABLE:
		apm_enet_slimpro_ipfw_enable(pdev,
					ioc->ioctl_ops.slimpro_ipfw_enable);
		break;
	case SIOSLIMPROIPFWIPADDR:
		apm_enet_slimpro_ipfw_set_ipaddr(pdev,
					ioc->ioctl_ops.slimpro_ipfw_ipaddr);
		break;
#endif
#if defined(CONFIG_INET_OFFLOAD)
	case SIOCGETHOFFLOAD:
		/* Get kernel ethoffload status */
		ioc->ioctl_ops.ethoffload = pdev->ethoffload;
		break;
#endif
	default:
