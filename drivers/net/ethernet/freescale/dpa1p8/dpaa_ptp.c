...
static int ptp_dpa_adjtime(struct ptp_clock_info *ptp, s64 delta)
{
	...
	if (mac_dev->fm_rtc_get_timer_offset)
		mac_dev->fm_rtc_get_timer_offset(mac_dev->fm_dev, &offset_now);

	offset_now += delta;

	if (mac_dev->fm_rtc_set_timer_offset)
		mac_dev->fm_rtc_set_timer_offset(mac_dev->fm_dev, offset_now);
...
static int ptp_dpa_gettime(struct ptp_clock_info *ptp, struct timespec *ts)
{
	...
	if (mac_dev->fm_rtc_get_timer_offset)
		mac_dev->fm_rtc_get_timer_offset(mac_dev->fm_dev, &offset);
			       ...
			       const struct timespec *ts)
{
	...
	if (mac_dev->fm_rtc_set_timer_offset)
		mac_dev->fm_rtc_set_timer_offset(mac_dev->fm_dev, 0);
