...
static void polling_timer_func(unsigned long dummy);

static DEFINE_TIMER(polling_timer, polling_timer_func, 0, 0);
#define POLLING_TIMER_PERIOD     (HZ / 100)
...
static int enc28j60_probe(struct spi_device *spi)
{
	...
	if (ret < 0) {
		if (netif_msg_probe(priv))
			dev_err(&spi->dev, DRV_NAME ": request irq %d failed (ret = %d), working with polling\n",
				spi->irq, ret);
		init_timer(&polling_timer);
		polling_timer.expires = jiffies + POLLING_TIMER_PERIOD;
		polling_timer.function = polling_timer_func;
		polling_timer.data = (unsigned long)priv;
...
static void polling_timer_func(unsigned long data)
{
	struct enc28j60_net *priv = (struct enc28j60_net *)data;

	schedule_work(&priv->irq_work);
	mod_timer(&polling_timer, jiffies + POLLING_TIMER_PERIOD);
}
