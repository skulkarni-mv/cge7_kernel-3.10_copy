...
void mv_pp2x_bm_bufs_free(struct device *dev, struct mv_pp2x *priv, struct mv_pp2x_bm_pool *bm_pool,
				int buf_num)
{
	int i;
...

{
	...
	return skb_tx_hash(dev, skb) % mv_pp2x_txq_number;
}
			    ...
			    struct mv_pp2x *priv)
{
	...
	/* Alloc per-cpu stats */
	port->stats = alloc_percpu(struct mv_pp2x_pcpu_stats);
	if (!port->stats) {
		err = -ENOMEM;
		goto err_free_irq;
	}
