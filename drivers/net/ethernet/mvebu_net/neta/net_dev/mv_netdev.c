...
int mv_eth_pool_free(int pool, int num)
{
	...
	if (mv_eth_pool_bm(ppool)) {
		...
		printk(KERN_ERR "bm pool #%d: pkt_size=%d, buf_size=%lu - %d of %d buffers free\n",
	...
	if (i > 0)
		printk(KERN_ERR "stack pool #%d: pkt_size=%d, buf_size=%lu - %d of %d buffers free\n",
			pool, ppool->pkt_size, RX_BUF_SIZE(ppool->pkt_size), i, num);
...
int mv_eth_pool_add(struct eth_port *pp, int pool, int buf_num)
{
	...
	if (bm_pool->pkt_size == 0) {
		printk(KERN_ERR "%s: invalid pool #%d state: pkt_size=%d, buf_size=%lu, buf_num=%d\n",
...
#endif
	printk(KERN_ERR "pool #%d: pkt_size=%d, buf_size=%lu - %d of %d buffers added\n",
...
void mv_eth_netdev_print(struct net_device *dev)
{
	...
	pr_info("ifIdx=%d, mtu=%u, pkt_size=%d, buf_size=%ld, MAC=" MV_MACQUAD_FMT "\n",
