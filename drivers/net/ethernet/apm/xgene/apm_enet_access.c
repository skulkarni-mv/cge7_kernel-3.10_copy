...
static void apm_enet_mdio_link_change(struct net_device *ndev)
{
	...
	if (phydev->link) {
		if (pdev->phy_speed != phydev->speed) {
			...
			if (phydev->speed != APM_ENET_SPEED_1000) {
...
static int apm_enet_mdio_probe(struct net_device *ndev)
{
	...
	ENET_PRINT("MDIO: %s: Phy Id 0x%08x, Driver \"%s\"\n",
...
process_pkt:
	...
	if (pdev->ndev->features & NETIF_F_GRO)
...
static int apm_enet_open(struct net_device *ndev)
{
	...
	if (!pdev->mdio_bus) {
		...
		schedule_delayed_work(&pdev->link_work, PHY_POLL_LINK_OFF);
...
static int apm_enet_qconfig(struct apm_enet_pdev *pdev)
{
	...
	for (qindex = 0; qindex < pdev->num_rx_queues; qindex++) {
		...
		apm_enet_clr_pb(&pdev->priv, qmtm_ip, qinfo.queue_id);
#ifdef CONFIG_XGENE_NET_JUMBO_FRAME
		...
		size_t count, loff_t *ppos)
{
	...
	if (cmd == APM_ENET_WRITE_CMD) {
		...
		if (block_id == BLOCK_ETH_INTPHY)
			apm_genericmiiphy_read(&enet_dev[port]->priv, INT_PHY_ADDR,
				reg_offset >> 2, &data);
		else if (block_id == BLOCK_ETH_EXTPHY) {
			struct apm_enet_priv *priv = &enet_dev[port]->priv;
			struct apm_enet_pdev *pdev =
				container_of(priv,
					struct apm_enet_pdev, priv);
			mutex_lock(&enet_dev[pdev->hw_config]->phy_lock);
...
static int apm_enet_of_probe(struct platform_device *platform_device)
{
	...
	port_addr = devm_ioremap(&platform_device->dev, port_addr_p,
				 resource_size(res));
	...
	ENET_DEBUG("Classifier VADDR: 0x%p\n", apm_class_base_addr[cid]);
#endif
	...
	for (i = 0; i < pdev->num_rx_queues; i++) {
		rx_dstqid[i] = QMTM_QUEUE_ID(pdev->qm_queues.qm_ip, pdev->qm_queues.rx[i].qid); 
		rx_fpsel[i] =  pdev->qm_queues.rx_fp[i].pbn;
	}
	apm_preclass_init(port_id, rx_dstqid, rx_fpsel);
	...
	apm_preclass_switch_tree(port_id, CLE_PTREE_DEFAULT);
#endif
...
_skip_mdio:
	...
	ENET_PRINT("APM X-Gene Ethernet device: %s (Mode: %s) probed.\n",
		ndev->name, pm);
	return APM_RC_OK;
}
