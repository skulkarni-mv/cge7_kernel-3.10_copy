		...
		u32 reg_offset, u32 value)
{
	...
	case BLOCK_XGENET_PCS:
		addr_reg_offst = priv->xgenet_pcs_addr_v + IND_ADDR_0_ADDR;
		cmd_reg_offst = priv->xgenet_pcs_addr_v + IND_COMMAND_0_ADDR;
		wr_reg_offst = priv->xgenet_pcs_addr_v + IND_WDATA_0_ADDR;
		cmd_done_reg_offst = priv->xgenet_pcs_addr_v
			+ IND_COMMAND_DONE_0_ADDR;
		indirect = 1;
		...
		u32 reg_offset, u32 *value)
{
	...
	case BLOCK_XGENET_PCS:
		addr_reg_offst = priv->xgenet_pcs_addr_v + IND_ADDR_0_ADDR;
		cmd_reg_offst = priv->xgenet_pcs_addr_v + IND_COMMAND_0_ADDR;
		rd_reg_offst = priv->xgenet_pcs_addr_v + IND_RDATA_0_ADDR;
		cmd_done_reg_offst = priv->xgenet_pcs_addr_v
			+ IND_COMMAND_DONE_0_ADDR;
		indirect = 1;
			   ...
			   unsigned char reg, u32 data)
{
	...
	PHY_PRINT("%s write MII_MGMT_ADDRESS_ADDR phy_id=0x%x, reg=0x%x, \
...
read_again:
	/* Write PHY number and address in MII Mgmt Address */
	value = PHY_ADDR_WR(phy_id) | REG_ADDR_WR(reg);
	PHY_PRINT("%s MII_MGMT_ADDRESS_ADDR phy_id=0x%x, reg=0x%x, \
			value=0x%x\n", __func__, phy_id, reg << 2, value);
	...
	PHY_PRINT("Read data=0X%x from MII_MGMT_STATUS_ADDR %s\n",
		       	*data, __func__);
...
void apm_enet_cle_bypass(struct apm_enet_priv *priv, u32 dstqid, u32 fpsel, u32 nxtfpsel, bool bypass_en)
{
	if (priv->cle_bypass)
