...
int sm_enet_module_init_enet_serdes(struct apm_enet_priv *priv, int rev)
{
	...
	if (!vco_calibration)
		ENET_PRINT("INIT_SERDES : PLL VCO Calibration %s\n",
	...
	if (!tx_ready || !rx_ready) {
		ENET_PRINT("INIT_SERDES : TX is %sready\n", tx_ready ? "" : "not ");
		ENET_PRINT("INIT_SERDES : RX is %sready\n", rx_ready ? "" : "not ");
	}
	return pll_ready;
}
...
void apm_enet_reset(struct apm_enet_priv *priv)
{
	...
	if ((priv->std_alone &&
		...
		//fscz apm_enet_serdes_init(priv);
	}
	//fscz gen_avg_val(priv, priv->port);
}
...
void apm_gmac_phy_link_mode(struct apm_enet_priv *priv, u32 *speed, u32 *state)
{
	...
	if (priv->phy_mode == PHY_MODE_SGMII) {
		...
		if (LINK_STATUS_RD(data) == 0) {/* No link wih SGMII */
			...
			*state = 0;
			*speed = priv->speed = APM_ENET_SPEED_10;
			return;
		}
		/* Get the final speed information from SGMII */
		apm_genericmiiphy_read(priv, INT_PHY_ADDR,
	...
	if (autoneg) {
		...
		if (LINK_STATUS_RD(data) == 0) {
			if (times ++ < 5) {
				...
				PHY_PRINT(".");
				apm_enet_write(priv, BLOCK_ETH_SDS_CSR,
					SATA_ENET_SDS_RST_CTL_ADDR, 0xDE);
				apm_enet_write(priv, BLOCK_ETH_SDS_CSR,
					SATA_ENET_SDS_RST_CTL_ADDR, 0xDF);
				goto reset_phy;
			}
		}
	} else {
		...
		int speed, int crc)
{
	...
	PHY_PRINT("\n %s writing speed %d \n", __FUNCTION__, speed);
	PHY_PRINT("\n %s writing rgmii_config_0 0x%x \n", __FUNCTION__, rgmii);
	PHY_PRINT("\n %s writing icm_config0 0x%x \n",
			__FUNCTION__, icm_config0);
	PHY_PRINT("\n %s writing icm_config2 0x%x \n",
			__FUNCTION__, icm_config2);
	PHY_PRINT("\n %s writing ecm_config0 0x%x \n",
			__FUNCTION__, ecm_config0);
	PHY_PRINT("\n %s writing mac_config2 0x%x \n",
			__FUNCTION__, mac_config_2);
	PHY_PRINT("\n %s writing interface control 0x%x \n",
