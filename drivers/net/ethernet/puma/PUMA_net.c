 ...
 * (Modified by Rupesh Kumar <rupeshk@mvista.com>
 * for MontaVista Software, LLC.)
 *
 * Modification Details:
 * Below mentioned parts have been modified to adapt to CGE7 kernel:
 * dma functions
 * Multicast address setting
 * adding net_device_ops
 * mdio bus allocation & phy_connect
...
MODULE_PARM_DESC(debug_level, "CPGMAC debug level (NETIF_MSG bits)");

/* Netif debug messages possible */
#define CPGMAC_EMAC_DEBUG    (NETIF_MSG_DRV | \
                NETIF_MSG_PROBE | \
                NETIF_MSG_LINK | \
                NETIF_MSG_TIMER | \
                NETIF_MSG_IFDOWN | \
...
static const char emac_version_string[] = "TI CPGMAC Linux v6.1";

#define    BD_ALIGNMENT    0xF
...
#define EMAC_DEF_RX_MAX_SERVICE        EMAC_POLL_WEIGHT /* should = netdev->weight */
...
/* Clock frequency for MDIO  */
#define MDIO_BUS_FREQ          25000000
#define MDIO_CLOCK_FREQ         2000000


/* EMAC internal utility function */ /* ARNAUD */
void __iomem * __remap_addr;

static inline u32 emac_virt_to_phys(void __iomem *addr)
{
    return ((u32 __force)(addr) - (u32 __force)(__remap_addr))+ 0x01E20000;
}

/* Cache macros - Packet buffers would be from skb pool which is cached */
#define EMAC_VIRT_NOCACHE(addr) (addr)
#define EMAC_CACHE_INVALIDATE(dev, addr, size) \
    dma_map_single(dev, (void *)addr, size, DMA_FROM_DEVICE)
#define EMAC_CACHE_WRITEBACK(dev, addr, size) \
    dma_map_single(dev, (void *)addr, size, DMA_TO_DEVICE)
#define EMAC_CACHE_WRITEBACK_INVALIDATE(dev, addr, size) \
    dma_map_single(dev, (void *)addr, size, DMA_BIDIRECTIONAL)
 ...
 * @priv: The CPGMAC private adapter structure
 *
 ...
 * @ndev: The CPGMAC network adapter
...
/**
 * ethtool_ops: CPGMAC Ethtool structure
 *
...
static int hash_add(struct emac_priv *priv, u8 *mac_addr)
{
    ...
    if (hash_value >= EMAC_NUM_MULTICAST_BITS) {
        if (netif_msg_drv(priv)) {
            dev_err(emac_dev, "CPGMAC: hash_add(): Invalid "\
...
static void emac_add_mcast(struct emac_priv *priv, u32 action, u8 *mac_addr)
{
    ...
    default:
        if (netif_msg_drv(priv))
            dev_err(emac_dev, "CPGMAC: add_mcast"\
...
static void emac_dev_mcast_set(struct net_device *ndev)
{
    ...
    if (ndev->flags & IFF_PROMISC) {
        ...
        if ((ndev->flags & IFF_ALLMULTI) ||
            (netdev_hw_addr_list_count(mcptr) > EMAC_DEF_MAX_MULTICAST_ADDRESSES)) {
            mbp_enable = (mbp_enable | EMAC_MBP_RXMCAST);
            emac_add_mcast(priv, EMAC_ALL_MULTI_SET, NULL);
        }
        if (netdev_hw_addr_list_count(mcptr) > 0) {
...
static int emac_init_txch(struct emac_priv *priv, u32 ch)
{
    ...
    if (NULL == txch) {
        dev_err(emac_dev, "CPGMAC: TX Ch mem alloc failed");
        return -ENOMEM;
    }
    priv->txch[ch] = txch;
    ...
    if (NULL == txch->tx_complete) {
        dev_err(emac_dev, "CPGMAC: Tx service mem alloc failed");
        kfree(txch);
        return -ENOMEM;
    }
    ...
    bd_size = (sizeof(struct emac_tx_bd) + BD_ALIGNMENT) & ~BD_ALIGNMENT;
    txch->num_bd = (priv->ctrl_ram_size >> 1) / bd_size;
    txch->alloc_size = (((bd_size * txch->num_bd) + BD_ALIGNMENT) & ~BD_ALIGNMENT);
    ...
    mem = (void __force __iomem *)
            (((u32 __force) txch->bd_mem + BD_ALIGNMENT) & ~BD_ALIGNMENT);
...
static int emac_tx_bdproc(struct emac_priv *priv, u32 ch, u32 budget)
{
    ...
    if (unlikely(1 == txch->teardown_pending)) {
        if (netif_msg_tx_err(priv) && net_ratelimit()) {
            dev_err(emac_dev, "CPGMAC:emac_tx_bdproc: "\
                "teardown pending\n");
        }
...
static int emac_send(struct emac_priv *priv, struct emac_netpktobj *pkt, u32 ch)
{
    ...
    /* FUTURE buff_ptr = dma_map_single(... data_ptr ...) */
...
static int emac_dev_xmit(struct sk_buff *skb, struct net_device *ndev)
{
    ...
    if (unlikely(!priv->link)) {
        if (netif_msg_tx_err(priv) && net_ratelimit())
            dev_err(emac_dev, "CPGMAC: No link to transmit");
    ...
    EMAC_CACHE_WRITEBACK(emac_dev, (unsigned long)skb->data, skb->len);
    ndev->trans_start = jiffies;
    ret_code = emac_send(priv, &tx_packet, EMAC_DEF_TX_CH);
    if (unlikely(ret_code != 0)) {
        if (ret_code == EMAC_ERR_TX_OUT_OF_BD) {
            if (netif_msg_tx_err(priv) && net_ratelimit())
                dev_err(emac_dev, "CPGMAC: xmit() fatal"\
                    " err. Out of TX BD's");
...
static void emac_dev_tx_timeout(struct net_device *ndev)
{
    ...
    if (netif_msg_tx_err(priv))
        dev_err(emac_dev, "CPGMAC: xmit timeout, restarting TX");
        ...
        void **data_token, u32 ch)
{
    ...
    if (unlikely(NULL == p_skb)) {
        if (netif_msg_rx_err(priv) && net_ratelimit())
            dev_err(emac_dev, "CPGMAC: failed to alloc skb");
        return NULL;
    }
    ...
    *data_token = (void *) p_skb;
    EMAC_CACHE_WRITEBACK_INVALIDATE(emac_dev, (unsigned long)p_skb->data, buf_size);
    return p_skb->data;
}
...
static int emac_init_rxch(struct emac_priv *priv, u32 ch, char *param)
{
    ...
    if (NULL == rxch) {
        dev_err(emac_dev, "CPGMAC: RX Ch mem alloc failed");
        return -ENOMEM;
    }
    priv->rxch[ch] = rxch;
    ...
    /* allocate buffer descriptor pool align every BD on four word
     * boundry for future requirements */
    bd_size = (sizeof(struct emac_rx_bd) + BD_ALIGNMENT) & ~BD_ALIGNMENT;
    rxch->num_bd = (priv->ctrl_ram_size >> 1) / bd_size;
    rxch->alloc_size = (((bd_size * rxch->num_bd) + BD_ALIGNMENT) & ~BD_ALIGNMENT);
    ...
    mem = (void __force __iomem *)
            (((u32 __force) rxch->bd_mem + BD_ALIGNMENT) & ~BD_ALIGNMENT);
    ...
    for (cnt = 0; cnt < rxch->num_bd; cnt++) {
        ...
        if (curr_bd->data_ptr == NULL) {
            dev_err(emac_dev, "CPGMAC: RX buf mem alloc " \
                "failed for ch %d\n", ch);
            kfree(rxch);
            return -ENOMEM;
        }
...
static void emac_setmac(struct emac_priv *priv, u32 ch, char *mac_addr)
{
    ...
    if (priv->rx_addr_type == 0) {
        ...
        if (netif_msg_drv(priv))
            dev_err(emac_dev, "CPGMAC: Wrong addressing\n");
    }
}
...
static int emac_dev_setmac_addr(struct net_device *ndev, void *addr)
{
    ...
    if (netif_msg_drv(priv))
        dev_notice(emac_dev, "CPGMAC: emac_dev_setmac_addr %pM\n",
                    priv->mac_addr);
        ...
        char *buffer, void *buf_token)
{
    ...
    /* populate the hardware descriptor */
    curr_bd->h_next = 0;
    /* FUTURE buff_ptr = dma_map_single(... buffer ...) */
              ...
              struct emac_netpktobj *net_pkt_list)
{
    ...
    EMAC_CACHE_INVALIDATE(&priv->ndev->dev, (unsigned long)p_skb->data, p_skb->len);
...
static int emac_poll(struct napi_struct *napi, int budget)
{
    ...
    if (status & mask)
    {
        num_tx_pkts = emac_tx_bdproc(priv, EMAC_DEF_TX_CH,EMAC_DEF_TX_MAX_SERVICE);
    } /* TX processing */

    priv->num_tx = priv->num_tx- num_tx_pkts;
    ...
    if (unlikely(status & mask)) {
        ...
        dev_err(emac_dev, "CPGMAC: Fatal Hardware Error\n");
        netif_stop_queue(ndev);
...
static int emac_mii_write(struct mii_bus *bus, int phy_id, int phy_reg, u16 phy_data)
{
    unsigned int phy_control;
...
static int emac_mii_reset(struct mii_bus *bus)
{
    ...
    emac_mdio_write(MDIO_CONTROL, MDIO_CONTROL_ENABLE | (clkdiv & MDIO_CONTROL_CLKDIV));

    return 0;

}

static int emac_dev_ioctl(struct net_device *ndev, struct ifreq *ifr, int cmd)
{
  struct emac_priv *priv = netdev_priv(ndev);
  struct mii_ioctl_data *data = (struct mii_ioctl_data *)if_mii(ifr);
  unsigned long flags;
  int ret_value = 0;

  spin_lock_irqsave(&priv->ioctl_lock, flags);

  switch (cmd)
  {
    case SIOCGMIIREG:
      emac_mii_reset(priv->mii_bus);  // To make sure that MDIO is setup correctly.
      data->val_out = emac_mii_read(priv->mii_bus, data->phy_id, data->reg_num);
      ret_value = 0;
      break;

    case SIOCSMIIREG:
      emac_mii_reset(priv->mii_bus);  // To make sure that MDIO is setup correctly.
      ret_value = emac_mii_write(priv->mii_bus, data->phy_id, data->reg_num, data->val_in);
      break;

    default:
      ret_value = -EOPNOTSUPP;
      break;
  }

  spin_unlock_irqrestore(&priv->ioctl_lock, flags);
...
static int emac_dev_open(struct net_device *ndev)
{
    ...
    if (0 != rc) {
        dev_err(emac_dev, "CPGMAC: emac_init_txch() failed");
        return rc;
    }
    rc = emac_init_rxch(priv, EMAC_DEF_RX_CH, priv->mac_addr);
    if (0 != rc) {
        dev_err(emac_dev, "CPGMAC: emac_init_rxch() failed");
        return rc;
    }
    /* Request IRQ */
    for (i = 0; i < 4; i++) {
	    ...
	    if (res != NULL) {
		    if (request_irq(res->start, emac_irq, IRQF_DISABLED, ndev->name, ndev)) {
			goto rollback;
		    }
	    }
    }
    ...
    if (priv->phy_mask)
       {
        ...
        priv->phydev = phy_connect(ndev, dev_name(&priv->phydev->dev),
                &emac_adjust_link, PHY_INTERFACE_MODE_MII);
    ...
    if (netif_msg_drv(priv))
        dev_notice(emac_dev, "CPGMAC: Opened %s\n", ndev->name);
...
rollback:
    dev_err(emac_dev, "CPGMAC: request_irq() failed");

    while (i--) {
	    res = platform_get_resource(priv->pdev, IORESOURCE_IRQ, i);
	    if (res != NULL)
		    free_irq(res->start, ndev);
    }
    return -EBUSY;
}
...
static int emac_dev_stop(struct net_device *ndev)
{
    ...
    if (netif_msg_drv(priv))
        dev_notice(emac_dev, "CPGMAC: %s stopped\n", ndev->name);
...
static struct net_device_ops CPGMAC_emac_netdev_ops = {
    .ndo_open       = emac_dev_open,
    .ndo_stop       = emac_dev_stop,
    .ndo_start_xmit     = emac_dev_xmit,
    .ndo_set_mac_address    = emac_dev_setmac_addr,
    .ndo_tx_timeout     = emac_dev_tx_timeout,
    .ndo_do_ioctl       = emac_dev_ioctl,
...
/**
 * CPGMAC_emac_probe: EMAC device probe
 * @pdev: The CPGMAC device that we are removing
 *
 * Called when probing for emac devicesr. We get details of instances and
 * resource information from platform init and register a network device
 * and allocate resources necessary for driver to perform
 */
static int __init CPGMAC_emac_probe(struct platform_device *pdev)
{
	int i, rc = 0;
	struct resource *res;
    struct net_device *ndev;
	...
	u32 ctrl_reg_offset, ctrl_mod_reg_offset, ctrl_ram_size;
	u32 mdio_reg_offset, ctrl_ram_offset;
#endif

    /* TODO: Probe PHY here if possible */
    ndev = alloc_etherdev(sizeof(struct emac_priv));
    if (!ndev) {
        printk(KERN_ERR "CPGMAC: Error allocating net_device\n");
        return -ENOMEM;
    }
    ...
    priv->pdev = pdev;
    priv->ndev = ndev;
    priv->msg_enable = netif_msg_init(debug_level, CPGMAC_EMAC_DEBUG);
    ...
    if (!pdata) {
        printk(KERN_ERR "CPGMAC: No platform data\n");
        return -ENODEV;
    }
...
#else
    /* TODO: handle for mac address */

	if (of_property_read_u32(np, "puma1,phy_mask", &priv->phy_mask))
		pr_warn("unable to get intc-size\n");
	if (of_property_read_u32(np, "puma1,rmii_en", (u32 *)&priv->rmii_en))
		pr_warn("unable to get rmii_en\n");
	if (of_property_read_u32(np, "puma1,version", (u32 *)&priv->version))
		pr_warn("unable to get version\n");
	priv->int_enable = NULL;
	priv->int_disable = NULL;
#endif

    priv->coal_intvl = 0;
    priv->bus_freq_mhz = (u32)(MDIO_BUS_FREQ / 1000000);
    ...
    if (!res) {
        dev_err(emac_dev, "CPGMAC: Error getting res\n");
        rc = -ENOENT;
        goto probe_quit;
    }
...
#else
	if (of_property_read_u32(np, "puma1,ctrl_reg_offset", &ctrl_reg_offset))
		pr_warn("unable to get ctrl_reg_offset\n");
	priv->emac_base_phys = res->start + ctrl_reg_offset;
#endif
    size = res->end - res->start + 1;
    if (!request_mem_region(res->start, size, ndev->name)) {
        dev_err(emac_dev, "CPGMAC: failed request_mem_region() \
                     for regs\n");
        rc = -ENXIO;
        goto probe_quit;
    }
    ...
    priv->remap_addr = ioremap(res->start, size);
       ...
       __remap_addr = (void *) ((u32) priv->remap_addr + 0x0300000);

       printk(KERN_ERR "END :: IO REMAP PHYS: 0x%08x -> VIRT: 0x%08x SIZE:%ld \n",res->start,(u32) priv->remap_addr,size);
...
#else
	priv->emac_base = priv->remap_addr + ctrl_reg_offset;
#endif
...
#else
	if (of_property_read_u32(np, "puma1,ctrl_mod_reg_offset",
					&ctrl_mod_reg_offset))
		pr_warn("unable to get ctrl_mod_reg_offset\n");
	if (of_property_read_u32(np, "puma1,ctrl_ram_size", &ctrl_ram_size))
		pr_warn("unable to get ctrl_ram_size\n");
	priv->ctrl_base = priv->remap_addr + ctrl_mod_reg_offset;
	priv->ctrl_ram_size = ctrl_ram_size;
#endif
...
#endif
    if (!res) {
        dev_err(emac_dev, "CPGMAC: Error getting irq res\n");
        rc = -ENOENT;
        goto no_irq_res;
    }
...
#else
	if (of_property_read_u32(np, "puma1,ctrl_ram_offset", &ctrl_ram_offset))
		pr_warn("unable to get ctrl_ram_offset\n");
	priv->emac_ctrl_ram = priv->remap_addr + ctrl_ram_offset;
#endif

#ifdef SCMB_I2C_E2P
    if (!is_valid_ether_addr(priv->mac_addr)) {
        /* Use random MAC if none passed */
        printk(KERN_WARNING "%s: BAD MAC addr: %pM\n",__func__, priv->mac_addr);
    ...
    random_ether_addr(priv->mac_addr);
#endif
    memcpy(ndev->dev_addr, priv->mac_addr, ETH_ALEN); // ARNAUD : ADDED
    ...
    ndev->netdev_ops = &CPGMAC_emac_netdev_ops;
    /* register the network device */
    SET_NETDEV_DEV(ndev, &pdev->dev);
    rc = register_netdev(ndev);
    if (rc) {
        dev_err(emac_dev, "CPGMAC: Error in register_netdev\n");
        rc = -ENODEV;
        goto netdev_reg_err;
    }
    ...
    if (emac_mii == NULL) {
        dev_err(emac_dev, "CPGMAC: Error allocating mii_bus\n");
        rc = -ENOMEM;
        goto mdio_alloc_err;
    }
...
#else
	if (of_property_read_u32(np, "puma1,mdio_reg_offset", &mdio_reg_offset))
		pr_warn("unable to get mdio_reg_offset\n");
	emac_mii->priv = priv->remap_addr + mdio_reg_offset;
#endif
    ...
    if (netif_msg_probe(priv)) {
        dev_notice(emac_dev, "CPGMAC Probe found device "\
               "(regs: %p, irq: %d)\n",
...
mdiobus_quit:
    printk(KERN_WARNING "MIDO BUS QUIT\n");
    kfree(emac_mii);

netdev_reg_err:
mdio_alloc_err:
no_irq_res:
    printk(KERN_WARNING "NETDEV QUIT\n");
    res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
    release_mem_region(res->start, res->end - res->start + 1);
    iounmap(priv->remap_addr);

probe_quit:
    printk(KERN_WARNING "PROBE QUIT\n");
    free_netdev(ndev);
    return rc;
}

/**
 * CPGMAC_emac_remove: EMAC device remove
 * @pdev: The CPGMAC device that we are removing
 *
 * Called when removing the device driver. We disable clock usage and release
 * the resources taken up by the driver and unregister network device
 */
static int __exit CPGMAC_emac_remove(struct platform_device *pdev)
{
    struct resource *res;
    struct net_device *ndev = platform_get_drvdata(pdev);
    struct emac_priv *priv = netdev_priv(ndev);

    dev_notice(&ndev->dev, "CPGMAC: CPGMAC_emac_remove()\n");
...
static struct of_device_id puma1_of_match[] = {
	{ .compatible = "puma1,ti-gpmac-ethernet", },
	{ /* End of List */ },
};
#endif
/**
 * CPGMAC_emac_driver: EMAC platform driver structure
 *
 * We implement only probe and remove functions - suspend/resume and
 * others not supported by this module
 */
static struct platform_driver CPGMAC_emac_driver = {
    .driver = {
        .name     = "CPGMAC",
        .owner     = THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = puma1_of_match,
#endif
    },
    .probe = CPGMAC_emac_probe,
    .remove = CPGMAC_emac_remove,
};


module_platform_driver(CPGMAC_emac_driver);


MODULE_LICENSE("GPL");
MODULE_AUTHOR("Windriver, Inc.");
MODULE_DESCRIPTION("CPGMAC Ethernet driver");
