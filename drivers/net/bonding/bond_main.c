...
static void bond_resend_igmp_join_requests(struct bonding *bond)
{
	...
	list_for_each_entry(vlan, &bond->vlan_list, vlan_list) {
		vlan_dev = __vlan_find_dev_deep_rcu(bond_dev, htons(ETH_P_8021Q),
						vlan->vlan_id);
		if (vlan_dev)
...
static int bond_has_this_ip(struct bonding *bond, __be32 ip)
{
	...
	list_for_each_entry(vlan, &bond->vlan_list, vlan_list) {
		rcu_read_lock();
		vlan_dev = __vlan_find_dev_deep_rcu(bond->dev, htons(ETH_P_8021Q),
						vlan->vlan_id);
		rcu_read_unlock();
...
static void bond_arp_send_all(struct bonding *bond, struct slave *slave)
{
	...
	for (i = 0; (i < BOND_MAX_ARP_TARGETS); i++) {
		...
		list_for_each_entry(vlan, &bond->vlan_list, vlan_list) {
			rcu_read_lock();
			vlan_dev = __vlan_find_dev_deep_rcu(bond->dev,
							htons(ETH_P_8021Q),
							vlan->vlan_id);
			rcu_read_unlock();
