...
static struct hc_driver __read_mostly opv5xc_xhci_plat_xhci_driver;
 ...
 *   # modprobe xhci-hcd
 * Check current mode:
 *   # cat /sys/kernel/debug/opv5xc/usb3h/u2testmode
 *   no test
 *   # echo test_packet > /sys/kernel/debug/opv5xc/usb3h/u2testmode
 * Check current mode:
 *   # cat /sys/kernel/debug/opv5xc/usb3h/u2testmode
 *   test_packet
 */
...
static int opv5xc_usb3h_debug_init(struct usb_hcd *hcd)
{
	/*
	 * Prepare debugfs directory for USB 3.0 host controller:
	 */
	...
	/* [DIR] /sys/kernel/debug/opv5xc/usb3h */
	root = debugfs_create_dir("usb3h", opv5xc_debugfs_dir);
	if (!root)
		goto err0;

	/* [FILE] /sys/kernel/debug/opv5xc/usb3h/regdump */
	if (!debugfs_create_file("regdump", S_IRUGO, root, hcd, &regdump_fops))
		goto err1;

	/* [FILE] /sys/kernel/debug/opv5xc/usb3h/u2testmode */
	if (!debugfs_create_file("u2testmode", S_IRUGO | S_IWUSR, root, hcd, &u2_testmode_fops))
		goto err1;

	/* [FILE] /sys/kernel/debug/opv5xc/usb3h/u3compliance */
...
static int  opv5xc_xhci_plat_probe(struct platform_device *pdev)
{
	...
	int			ret;
	int			irq;
	struct completion xhci_complete;
	...
	writel((readl(OPV5XC_CR_PMU_BASE_VIRT)        & ~(1 << 16)), OPV5XC_CR_PMU_BASE_VIRT);
	
	/* Software reset, low active */
	writel((readl(OPV5XC_CR_PMU_BASE_VIRT + 0x04) & ~(1 << 16)), OPV5XC_CR_PMU_BASE_VIRT + 0x04);
	writel((readl(OPV5XC_CR_PMU_BASE_VIRT + 0x04) |  (1 << 16)), OPV5XC_CR_PMU_BASE_VIRT + 0x04);
	...
	do {
		wait_for_completion_timeout(&xhci_complete, timeout);
		if (readl(OPV5XC_CR_PMU_BASE_VIRT + 0x10) & (1<<16)) {
			val = 1;
			break;
		}
	} while (counter-- != 0);
	if ((counter == 0) && (val == 0)) {
		printk("\n: Timeout while enabling power for XHCI-HCD \n");
		return -ENODEV;
	}
	...
	writel(0x999EC000, OPV5XC_MISC_BASE_VIRT + 0x814);
	writel(0x18E4000A, OPV5XC_MISC_BASE_VIRT + 0x818);
#endif

	if (usb_disabled())
		return -ENODEV;

	driver = &opv5xc_xhci_plat_xhci_driver;
...
#if defined(CONFIG_ARCH_OPV5XC_ES1) || defined(CONFIG_ARCH_OPV5XC_ES2)
	/*
	 * Clear GUSB2PHYCFGn bit 6 to prevent USB suspend early before device
	 * connected.
	 * GUSB2PHYCFGn bit 6 description
	 * Suspend USB2.0 HS/FS/LS PHY (SusPHY)
	 * When set, USB2.0 PHY enters Suspend mode if Suspend conditions are valid.
	 */
	reg = readl(hcd->regs + DWC3_GUSB2PHYCFG(0));
	reg &= ~DWC3_GUSB2PHYCFG_SUSPHY;
	writel(reg, hcd->regs + DWC3_GUSB2PHYCFG(0));
#endif

#ifdef CONFIG_DEBUG_FS
	opv5xc_usb3h_debug_init(hcd);
#endif
...
static int opv5xc_xhci_plat_remove(struct platform_device *dev)
{
...
#ifdef CONFIG_PM
static int opv5xc_xhci_plat_suspend(struct platform_device *pdev)
{
	struct xhci_hcd         *xhci;
	struct usb_hcd          *hcd;
	...
	hcd = dev_get_drvdata(&pdev->dev);
	xhci = hcd_to_xhci(hcd);
	xhci_suspend(xhci, device_may_wakeup(&pdev->dev));
	return 0;
}

static int opv5xc_xhci_plat_resume(struct platform_device *pdev)
{
	struct xhci_hcd         *xhci;
	struct usb_hcd          *hcd;
	uint32_t 		 reg;
	...
	*/
	...
	xhci_resume(xhci, 0);	/* 1 - denotes resumption from hibernation */
	return 0;
}
#endif

static struct platform_driver opv5xc_usb_xhci_driver = {
	.probe	= opv5xc_xhci_plat_probe,
	.remove	= opv5xc_xhci_plat_remove,
	.driver	= {
		.name = "opv5xc-xhci",
	},
#ifdef CONFIG_PM
	.suspend = opv5xc_xhci_plat_suspend,
	.resume  = opv5xc_xhci_plat_resume,
#endif
};

static void opv5xc_xhci_plat_quirks(struct device *dev, struct xhci_hcd *xhci)
{
	/*
...
static int opv5xc_xhci_plat_setup(struct usb_hcd *hcd)
{
	return xhci_gen_setup(hcd, opv5xc_xhci_plat_quirks);
}

static const struct xhci_driver_overrides opv5xc_xhci_overrides __initconst = {
	.extra_priv_size = sizeof(struct xhci_hcd),
	.reset = opv5xc_xhci_plat_setup,
};

static int __init opv5xc_xhci_register_plat_init(void)
{
	xhci_init_driver(&opv5xc_xhci_plat_xhci_driver, &opv5xc_xhci_overrides);
	return platform_driver_register(&opv5xc_usb_xhci_driver);
}
module_init(opv5xc_xhci_register_plat_init);

static void __exit opv5xc_xhci_unregister_plat_exit(void)
{
	platform_driver_unregister(&opv5xc_usb_xhci_driver);
}
module_exit(opv5xc_xhci_unregister_plat_exit);

MODULE_DESCRIPTION("xHCI-opv5xc Platform Host Controller Driver");
MODULE_LICENSE("GPL");
