...
#define READ_XADC(dev, reg) readl((dev->iobase + XADC_##reg))
#define WRITE_XADC(dev, reg, value) writel(value, dev->iobase+XADC_##reg)
...
static void run_batch(struct xadc_t *xadc)
{
	...
	WRITE_XADC(xadc, CONFIG, config);

	/* unmask CFIFO,DFIFO interrupts */
	WRITE_XADC(xadc, INTMSK, READ_XADC(xadc, INTMSK) &
...
static irqreturn_t xadc_irq(int irq, void *data)
{
	...
	if (intsts & ~intmsk & XADC_INT_DFIFO_GTH) {
		...
		WRITE_XADC(xadc, INTSTS, XADC_INT_DFIFO_GTH);
	}

	if (intsts & ~intmsk & XADC_INT_CFIFO_LTH) {
		int i, towrite;
		u32 status = READ_XADC(xadc, STATUS);
		...
		WRITE_XADC(xadc, INTSTS, XADC_INT_CFIFO_LTH);
	}
...
static int xadc_probe(struct platform_device *pdev)
{
	...
	WRITE_XADC(xadc, CONFIG, 0);
	WRITE_XADC(xadc, CTL, 0); /* ~RESET */

	WRITE_XADC(xadc, CONFIG, XADC_CONFIG_WEDGE | /* Default values */
		XADC_CONFIG_REDGE |
		SETFIELD(XADC_CONFIG, TCKRATE, TCKRATE_DIV16) |
		SETFIELD(XADC_CONFIG, IGAP, 20));

	WRITE_XADC(xadc, CONFIG, READ_XADC(xadc, CONFIG) | XADC_CONFIG_ENABLE);

	WRITE_XADC(xadc, INTSTS, ~0); /* clear all interrupts */
	WRITE_XADC(xadc, INTMSK, ~0); /* mask all interrupts */
