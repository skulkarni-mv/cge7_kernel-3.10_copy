...
#define to_puma_nand(m) container_of(m, struct puma_nand_info, mtd)
...
static int nand_puma_waitfunc(struct mtd_info *mtd, struct nand_chip *this)
{
    ...
    /* Since the NAND R/B# signal is not connected and it's not possible to check R/B# status reading it
     * in STATUS REGISTER because this operation will require issuing a READ STATUS command (0x70)
     * breaking the command sequence of the ongoing operation, we will wait for the ready in the waitfunc,
     * i.e. at the end of the operation.
     * NOTE: waiting for the ready is crucial in operations that touch data inside NAND block because,
     *       these operations MUST be ended before proceeding with further operations and, moreover, they require
     *       a not predictable amount of time so it's not possible to use a predefined timeout. */
...
static int __init nand_puma_probe(struct platform_device *pdev)
{
...
#endif
	info->mtd.name          = mtd_name ? : dev_name(&pdev->dev);
    ...
    if (pdata->ecc_bits == 4) {
         ...
         */
        dev_warn(&pdev->dev, "no 4-bit ECC support yet for 4KiB-page NAND\n");
        ret = -EIO;
        goto err_scan;
...
syndrome_done:
    ...
    if (mtd_has_partitions()) {
		...
		if (ret)
			dev_err(&pdev->dev, "pdata->parts : Not Found\n");
