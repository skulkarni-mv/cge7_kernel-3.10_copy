...
static MV_STATUS mvNfcReadIdNative(MV_NFC_CHIP_SEL cs, MV_U32 *id);
...
MV_STATUS mvNfcInit(MV_NFC_INFO *nfcInfo, MV_NFC_CTRL *nfcCtrl, struct MV_NFC_HAL_DATA *halData)
{
	...
	ctrl_reg |= (0x4 << NFC_CTRL_RD_ID_CNT_OFFS);
	...
	for (i = 0; i < (sizeof(flashDeviceInfo) / sizeof(MV_NFC_FLASH_INFO)); i++) {
		if (flashDeviceInfo[i].id == (read_id & 0xFFFF)) {
			/*Compare the page size : Byte3, 0 & 1st bit*/
			MV_U16 pgsz = (read_id >> 24) & 0x03;
			if (flashDeviceInfo[i].pgSz == (1024 << pgsz))
				break;
		}
	}
...
static MV_STATUS mvNfcReadIdNative(MV_NFC_CHIP_SEL cs, MV_U32 *id)
{
	...
	*id = MV_NAND_REG_READ(NFC_DATA_BUFF_REG);
