			    ...
			    int column, int page_addr)
{
	...
	case NAND_CMD_RNDIN:
		/*
		 * RNDIN requires tADL for some commands, tCCS for others.
		 * Choose the max delay here to accomodate both.
		 */
		ndelay(max(chip->tadl_ns, chip->tccs_ns));
		return;
					...
					int *busw)
{
	...
	chip->tadl_ns = (uint32_t)le16_to_cpu(p->t_adl);
	chip->tccs_ns = (uint32_t)le16_to_cpu(p->t_ccs);
	*busw = 0;
						  ...
						  struct nand_flash_dev *type)
{
	...
	*dev_id = chip->read_byte(mtd);
	...
	/* Default values of tCCS and tADL are 1us */
	chip->tccs_ns = 1000;
	chip->tadl_ns = 1000;
