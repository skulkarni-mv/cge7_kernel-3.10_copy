...
iproc_nand_get_cfg(struct iproc_nand_host *host, struct iproc_nand_cfg *cfg)
{
	...
	ret = of_property_read_u32(np, "block-size", &cfg->block_size);
	if(ret < 0)
		cfg->block_size = RD_CONFIG(host->cs, BLOCK_SIZE);

	ret = of_property_read_u64(np, "device-size", &cfg->device_size);
	if(ret < 0)
		cfg->device_size = (4ULL << 20) << RD_CONFIG(host->cs, DEVICE_SIZE);

	ret = of_property_read_u32(np, "page-size", &cfg->page_size);
	if(ret < 0)
		cfg->page_size = RD_CONFIG(host->cs, PAGE_SIZE);

	ret = of_property_read_u32(np, "device-width", &cfg->device_width);
	if(ret < 0)
		cfg->device_width = RD_CONFIG(host->cs, DEVICE_WIDTH) ? 16 : 8;

	ret = of_property_read_u32(np, "col-adr-bytes", &cfg->col_adr_bytes);
	if(ret < 0)
		cfg->col_adr_bytes = RD_CONFIG(host->cs, COL_ADR_BYTES);

	ret = of_property_read_u32(np, "blk-adr-bytes", &cfg->blk_adr_bytes);
	if(ret < 0)
		cfg->blk_adr_bytes = RD_CONFIG(host->cs, BLK_ADR_BYTES);

	ret = of_property_read_u32(np, "ful-adr-bytes", &cfg->ful_adr_bytes);
	if(ret < 0)
		cfg->ful_adr_bytes = RD_CONFIG(host->cs, FUL_ADR_BYTES);

	ret = of_property_read_u32(np, "spare-area-size", &cfg->spare_area_size);
	if(ret < 0)
		cfg->spare_area_size = RD_ACC_CONTROL(host->cs, SPARE_AREA_SIZE);

	ret = of_property_read_u32(np, "sector-size-1k", &cfg->sector_size_1k);
	if(ret < 0)
		cfg->sector_size_1k = RD_ACC_CONTROL(host->cs, SECTOR_SIZE_1K);

	ret = of_property_read_u32(np, "ecc-level", &cfg->ecc_level);
	if(ret < 0)
