...
#define	USE_TMP_4BYTE		0x100	/* use temporary setting of 4byte mode */
#define SPI_NOR_DDR_QUAD_READ	0x200	/* Flash supports DDR Quad Read */
};
...
#ifdef CONFIG_MTD_SPI_NOR_TMP_4BYTE
	if ((ops == SPI_NOR_OPS_WRITE || ops == SPI_NOR_OPS_ERASE) &&
	    (nor->flags & SNOR_F_TMP_4BYTE))
		set_4byte(nor, 1);
#endif
...
static const struct spi_device_id spi_nor_ids[] = {
	...
	{ "n25q256a",    INFO(0x20ba19, 0, 64 * 1024,  512, SECT_4K | SPI_NOR_QUAD_READ | USE_TMP_4BYTE) },
	{ "n25q512a",    INFO(0x20bb20, 0, 64 * 1024, 1024, SECT_4K | USE_FSR | SPI_NOR_QUAD_READ | USE_TMP_4BYTE) },
	{ "n25q512ax3",  INFO(0x20ba20, 0, 64 * 1024, 1024, SECT_4K | USE_FSR | SPI_NOR_QUAD_READ | USE_TMP_4BYTE) },
...
int spi_nor_scan(struct spi_nor *nor, const char *name, enum read_mode mode)
{
	...
	if (info->addr_width)
		...
		if (JEDEC_MFR(info) == CFI_MFR_AMD) {
			...
			mtd->erasesize = info->sector_size;
#ifdef CONFIG_MTD_SPI_NOR_TMP_4BYTE
		} else if (info->flags & USE_TMP_4BYTE) {
			nor->flags |= SNOR_F_TMP_4BYTE;
#endif
		} else
			set_4byte(nor, 1);
