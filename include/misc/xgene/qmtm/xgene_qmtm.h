...
struct xgene_qmtm_qinfo {
	...
	u32 queue_id;		/* ret, queue id */
	u32 headptr;		/* ret, HeadPtr in the queue */
	u32 nummsgs;		/* ret, number of messages in the queue */
...
/* QM SAB callback function type */
typedef void (*xgene_qmtm_sab_fn_ptr) (void *data);

/* QM MOD SAB command */
#define XGENE_QMTM_MODSAB_CMD(thr, cpusab, crid, qne, en) ((en & 0x1) | \
							((qne & 0x1) << 1) | \
							((crid & 0x1) << 2) | \
							((cpusab & 0xff) << 3) | \
							((thr & 0x7) << 11))
...
void xgene_qmtm_set_pb(struct xgene_qmtm_qinfo *qinfo);

void xgene_qmtm_clr_pb(struct xgene_qmtm_qinfo *qinfo);
...
/*
 * @brief   Request and free sab irq
 * @param   qmtm_ip - slave belongs to QMTM0, QMTM1, QMTM2 or QMTM3
 *          qid - QID of the queue for sab is requested
 *          entry - funtion pointer to call when queue enters to critical region
 *          exit - funtion pointer to call when queue exits from critical region
 *          data - parameter to pass to entry/exit routine
 */

void xgene_qmtm_request_sab_irq(u32 qmtm_ip, u32 qid, xgene_qmtm_sab_fn_ptr entry,
                                xgene_qmtm_sab_fn_ptr exit, void *data);
