 ...
 */
int fsl_upm_get_device(struct fsl_upm *upm, int new_state)
{
...
retry:
	...
	if (fsl_lbc_ctrl_dev->active_upm == upm &&
	    fsl_lbc_ctrl_dev->upm_state == UPM_READY) {
		fsl_lbc_ctrl_dev->upm_state = new_state;
 ...
 */
int fsl_upm_try_get_device(struct fsl_upm *upm, int new_state)
{
	if (fsl_lbc_ctrl_dev->active_upm == upm)
		return 0;

	return fsl_upm_get_device(upm, new_state);
}
