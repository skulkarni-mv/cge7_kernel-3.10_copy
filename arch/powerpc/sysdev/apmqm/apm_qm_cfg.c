...
int qm_cfg_add_qconfig(struct qm_cfg_qconfig * qconfig)
{
	...
	qalloc.qaddr     = (qconfig->qaddr != 0xFFFFFFFF) ? qconfig->qaddr : 0;
	...
	if (qconfig->qtype == QM_CFG_QTYPE_VQ) {
		...
		if (qconfig->qcount == 0 || qconfig->qcount > QM_CFG_MAX_QSEL) {
...
EXPORT_SYMBOL(qm_cfg_add_qconfig);
...
EXPORT_SYMBOL(qm_cfg_delete_qconfig);
...
EXPORT_SYMBOL(qm_cfg_dev_notify_register);
...
int qm_cfg_dev_stop(u16 dev)
{
	...
	list_for_each_entry(entry, &qm_cfg_dev_notify_list, node)
		if (entry->notify->dev == dev &&
			...
			return entry->notify->stop(entry->notify->priv_data);
		}
...
int qm_cfg_dev_start(u16 dev)
{
	...
	list_for_each_entry(entry, &qm_cfg_dev_notify_list, node)
		if (entry->notify->dev == dev && entry->notify->start != NULL) {
			return entry->notify->start(entry->notify->priv_data);
		}
...
int qm_cfg_dev_set_qgroup(struct qm_cfg_qgroup * qgroup)
{
	...
	list_for_each_entry(entry, &qm_cfg_dev_notify_list, node)
		if (entry->notify->dev == qgroup->dev &&
			...
			return entry->notify->set_qgroup(entry->notify->priv_data,
					qgroup);
		}
...
int qm_cfg_get_qgroup(struct qm_cfg_qgroup * qgroup)
{
	...
	list_for_each_entry(entry, &qm_cfg_qgroup_list, node) {
		if (strcmp(entry->qgroup.name, qgroup->name) == 0 &&
			...
			memcpy(qgroup, &entry->qgroup, sizeof(struct qm_cfg_qgroup));
			return 0;
		}
	}
...
EXPORT_SYMBOL(qm_cfg_add_qgroup);
			...
			u32 cmd, unsigned long arg)
{
	...
	case QM_CFG_GET_VERSION:
		data = QM_CFG_VERSION;
		ret = copy_to_user((u32*)arg, &data, sizeof(u32));
		break;
	...
	case QM_CFG_GET_QCONFIG:
		...
		if (rc == 0)
			ret = copy_to_user((struct qm_cfg_qconfig *)arg, &qconfig,
				sizeof(struct qm_cfg_qconfig));
		break;
	...
	case QM_CFG_GET_QGROUP:
		...
		if (rc == 0)
			ret = copy_to_user((struct qm_cfg_qgroup *)arg, &qgroup,
				sizeof(struct qm_cfg_qgroup));
		break;
	...
	case QM_CFG_GET_STAT_CFG:
		...
		ret = copy_to_user((struct qm_cfg_stat_cfg *)arg, &data,
	...
	case QM_CFG_GET_STAT:
		...
		ret = copy_to_user((struct qm_cfg_stat *)arg, &stat,
...
struct file_operations qm_cfg_fops = {
	...
	.read    = qm_cfg_read,
	.write   = qm_cfg_write,
	.unlocked_ioctl   = qm_cfg_ioctl,
};

int __init qm_cfg_init(void)
{
	struct proc_dir_entry *entry;
	printk(KERN_INFO "QM Configuration Manager v%d\n", QM_CFG_VERSION);

	entry = proc_create(QM_CFG_NAME, 0644, NULL, &qm_cfg_fops);
	if (entry == NULL) {
