...
/* Determine if Hardware is an APM864xx variant */
int is_apm864xx(void);

/* Determine if Hardware is an APM866xx variant */
...
int ipp_send_addr_msg(enum ipp_addr_message_handlers hdlr,
			u32 *addr_buf, int len, u32 *retcode);
...
#define REG_GROUP_SRST_CLKEN_MAX	2
...
/* READ register - returns 0 if successful      */
/* bankid: Reg bank defined in apm_ipp_interface.h */
/* offset: Reg offset defined in apm_ipp_csr.h */
/* Use with caution: meant for debugging only       */
int secure_read_reg(enum ipp_regbank_id bankid, u32 offset, u32 *val);

/* WRITE register - returns 0 if successful      */
/* bankid: Reg bank defined in apm_ipp_interface.h */
/* offset: Reg offset defined in apm_ipp_csr.h */
/* mask: Reg mask */
/* Use with caution: meant for debugging only       */
...
/* offset: SCU reg offset defined in apm_ipp_csr.h */
/* Use with caution: meant for debugging only       */
...
/* CLEAR MPA register bit with mask */
int apm86xxx_write_mpa_reg_clrmask(u32 offset, u32 mask);
/* SET MPA register bit with mask */
int apm86xxx_write_mpa_reg_setmask(u32 offset, u32 mask);

/* SET SCU register bit with mask */
int apm86xxx_write_scu_reg_clrmask(u32 offset, u32 mask);

/* CLEAR SCU register bit with mask */
int apm86xxx_write_scu_reg_clrmask(u32 offset, u32 mask);

/* Returns non-zero (TRUE) value if apm86xxx is in secure mode */
/* Returns zero (FALASE) value if apm86xxx is in normal mode */
/* Secure mode means the SCU and MPA registers are protected */
 ...
 * 
 * This function call be called by either arch_init call or by earlier module
 */
