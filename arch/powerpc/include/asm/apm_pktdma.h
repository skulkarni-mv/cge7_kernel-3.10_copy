...
#define IODMA_MAX_BUF_SIZE	(16*1024) /* Max size of a buffer */
#define PKTDMA_CHID_MAX		4	  /* Total hardware channel */	
...
struct dma_chan_info {
	int max_chan;
	int chan_en[PKTDMA_CHID_MAX];
	int chan_rsvd[PKTDMA_CHID_MAX];	/* indicate channel is reserved */
};
 ...
 * @param   fp_buf      Get a free pool buffer
 */
 ...
 * @param   chan_info   dma_chan_info  structure pointer
 */
void apm_pktdma_get_total_chan(struct dma_chan_info *chan_info);

/**
 * @brief   Retrieve Queue ID of a channel
 * @param   chid	Channel id
 * @param   cos		Channel CoS
 * @return  Queue id or 0 for invalid queue
 */
int apm_pktdma_chan2qid(int chid, int cos);

/**
 * @brief   Rountine to flush the dest
 * @param   daddr	physical destination pointer 
 * @param   byte_cnt	Size to flush
 */
void apm_pktdma_flush_dest(void *daddr, int byte_cnt);

/**
 * @brief   Rountine to invalidate the dest
 * @param   da		virtual destination pointer
 * @param   byte_cnt	Size to flush
 */
 ...
 * @param   saddr	virtual destination pointer
 * @param   byte_cnt	Size to flush
 */
void apm_pktdma_flush_src(void *saddr, int byte_cnt);

/**
 * @brief   Reserve PktDMA descriptor
 * @param   chanid 	Channel id
 * @param   slot	Number of slot to reserve
 * 
 * In situation where a channel slot is required for use by SlimPRO,
 * this function can reserve slot accordingly and not report them to upper
 * layer.
 */
/* void apm_pktdma_reserve_slot(int chanid, int slot); */
 
/**
 * @brief Dump PktDMA QMI registers
 *
 */ 
