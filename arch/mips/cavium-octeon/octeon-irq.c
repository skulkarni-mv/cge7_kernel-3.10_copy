...
static void octeon_irq_ciu3_ip2(void)
{
	...
	if (likely(dest_pp_int.s.intr)) {
		...
		/*
		 * Microstate accounting requires semi-valid regs so
		 * it can know if this came from userland or kernel.
		 * Supply regs good enough to accomplish this.
		 */
		struct pt_regs dummy_regs;
		struct pt_regs *regs = &dummy_regs;
		...
		ret = handle_domain_irq(domain, hw, regs);
		if (ret < 0) {
			union cvmx_ciu3_iscx_w1c isc_w1c;
