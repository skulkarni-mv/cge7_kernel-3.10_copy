...
static inline bool thread_in_livedump(struct task_struct *p)
{
	return livedump_task_dump(p) != NULL;
}
#else
static inline bool thread_in_livedump(struct task_struct *p) { return false; }
#endif
				 ...
				 int trace)
{
	...
	*/
	...
	if (p->pid && !thread_group_leader(p))
		p->group_leader = current->group_leader;
