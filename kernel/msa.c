...
asmlinkage void msa_kernel(void)
{
	...
	WARN_ONCE(!irqs_disabled(), "msa_kernel() called with interrupts enabled\n");
...
asmlinkage void msa_user(void)
{
	...
	WARN_ONCE(!irqs_disabled(), "msa_user() called with interrupts enabled\n");
...
void msa_irq_exit(int irq_id, int is_going_to_user)
{
	...
	if (!nested) {
		...
		cpustat[CPUTIME_IRQ] += msa_to_cputime(delta);
		...
		if (now < before)
			...
			delta = now - before;
		cpustat[CPUTIME_SOFTIRQ] += msa_to_cputime(delta);
